...
# Manually adjusted stylistic elements
styles = {
    'marker': ['^', 's', 'x', 'd'],  # different markers
...
for idx, region in enumerate(regions):
    ...
    for p_idx, product in enumerate(products):
        ax.plot(months, sales_data[region][product], marker=styles['marker'][p_idx],
                linestyle=styles['linestyle'][p_idx], color=colors[p_idx], label=product)
    
    # Randomized grid feature
    ax.grid(color='grey', linestyle=styles['linestyle'][idx % 4], linewidth=0.5)
    ax.set_title(region)
    ax.legend(fontsize=8, loc='upper left')  # adjusted legend
