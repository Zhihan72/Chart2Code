...
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 14))  # Changed from (1, 2) to (2, 1)

ax1.stackplot(years, ev_adoption, colors=['#1f78b4'], alpha=0.8)
ax1.set_title('EV Surge Across Continents (2010-2025)', fontsize=16, fontweight='bold')
ax1.set_xlabel('Timeline', fontsize=14)
ax1.set_ylabel('EV Count (millions)', fontsize=14)
ax1.set_xticks(years)
ax1.set_yticks(np.arange(0, 20 + 1, 2))

for i, continent in enumerate(['N. America', 'Europa', 'Asiatic', 'S. America', 'Afrika', 'Oceania', 'Atlantia', 'Lemurian']):
    ax2.plot(years, percentage_shares[i], color='#1f78b4', marker='o')

ax2.set_title('Continent EV Shares (2010-2025)', fontsize=16, fontweight='bold')
ax2.set_xlabel('Years', fontsize=14)
ax2.set_ylabel('Percentage Share (%)', fontsize=14)
