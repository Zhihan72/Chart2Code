...
bar_width = 0.4
ax1.bar(years, revenues, width=bar_width, color='darkorange', label='Income (Billion $)')
...
ax2 = ax1.twinx()
ax2.plot(years, profits, color='maroon', marker='x', linestyle='--', linewidth=2, label='Earnings (Billion $)')
ax2.set_ylabel('Net Income (B$)', fontsize=14, color='maroon')
for i, profit in enumerate(profits):
    ax2.text(years[i], profit + 0.2, f'{profit:.1f}', color='maroon', ha='center')

ax3 = ax1.twinx()
ax3.spines['right'].set_position(('outward', 60))
ax3.plot(years, market_shares, color='seagreen', marker='v', linestyle='-', linewidth=2, label='Market Dominance (%)')
ax3.set_ylabel('Dominance (%)', fontsize=14, color='seagreen')
