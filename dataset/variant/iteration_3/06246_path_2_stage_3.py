...
# Stacked bar chart with diverging concept
...
for idx, deviation in enumerate(rainfall_dev):
    ax.bar(months, deviation, bottom=mean_rainfall, color=color_rainfall[idx], alpha=0.7, label=f'{cities[idx]} Rainfall Deviation')

ax.set_title('Diverging Rainfall Deviation From Mean\nAcross Major Cities', fontsize=18, fontweight='bold')
ax.set_xlabel('Months', fontsize=14)
ax.set_ylabel('Deviation in Rainfall (mm)', fontsize=14)
ax.axhline(y=0, color='gray', linewidth=1)
ax.yaxis.grid(True, linestyle='--', color='gray', alpha=0.7)
ax.legend(loc='upper left', title='Cities', bbox_to_anchor=(1.02, 1), borderaxespad=0.)
