...
# Funnel Plot - using different edge styles and adding dashed grid
...
for i in range(len(stages) - 1):
    ...
    if i < len(stages) - 2: 
        trapezoid = patches.Polygon(
            ...
            closed=True, color=colors[i], edgecolor='black', linestyle='-.', linewidth=1.5
        )
        ax1.add_patch(trapezoid)
left = (max(user_counts) - user_counts[-1]) / 2
rectangle = patches.Rectangle(
    (left, len(stages) - 1), user_counts[-1], 1, color=colors[-1], edgecolor='black', linestyle='-.', linewidth=1.5
)
ax1.add_patch(rectangle)
for i, (stage, count) in enumerate(zip(stages, user_counts)):
    ax1.text(max(user_counts) / 2, i + 0.5, f'{stage}: {count}', va='center', ha='center', color='black', fontsize=12, fontweight='bold')
ax1.set_xticks([])
ax1.set_yticks([])
