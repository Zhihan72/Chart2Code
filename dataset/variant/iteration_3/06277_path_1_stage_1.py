...
# Additional Chart: Visualizing geothermal energy production to compare with solar, wind, and hydro energy production.
...
# Adding made-up data for geothermal energy production
...
# Bar chart for Geothermal Energy
for i in range(len(countries)):
    axes[3].bar(
        years, geothermal_energy[i], label=countries[i]
    )
axes[3].set_title('Annual Geothermal Energy Production (2017-2021)', fontsize=16, fontweight='bold')
axes[3].set_xlabel('Years', fontsize=12)
axes[3].set_ylabel('Energy (TWh)', fontsize=12)
axes[3].legend(title='Countries', fontsize=10, loc='upper left', bbox_to_anchor=(1, 1), title_fontsize='12')
