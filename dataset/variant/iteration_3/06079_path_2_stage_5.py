...
axs[0].plot(months, city_c_temps, ':o', color='teal', label='C', linewidth=2, markerfacecolor='darkorange')
axs[0].plot(months, city_e_temps, '--v', color='crimson', label='E', linewidth=2, markerfacecolor='gold')
...
axs[0].axhline(y=winter_mean, color='slateblue', linestyle=':', label='W Mean')
axs[0].axhline(y=spring_mean, color='olive', linestyle='-.', label='Sp Mean')
axs[0].axhline(y=summer_mean, color='darkred', linestyle='-', label='Su Mean')
axs[0].axhline(y=autumn_mean, color='chocolate', linestyle='-', label='A Mean')
...
for season, mean_temp in season_annot:
    axs[0].text(len(months)-1, mean_temp + 0.5, f'{season} mean: {mean_temp:.1f}°C', 
                fontsize=8, color='darkblue', ha='right', backgroundcolor='pink')

axs[1].plot(months, city_a_temps, '-s', color='skyblue', label='A', linewidth=2, markerfacecolor='salmon')
axs[1].plot(months, city_b_temps, '-^', color='darkorchid', label='B', linewidth=2, markerfacecolor='salmon')
axs[1].plot(months, city_d_temps, ':p', color='limegreen', label='D', linewidth=2, markerfacecolor='salmon')
