...
ax.stackplot(years, data, labels=['Power', 'Commute', 'Manufacturing', 'Farming', 'Innovation'], 
             colors=['#ff9999','#66b3ff','#99ff99','#ffcc99','#c2c2f0'], alpha=0.7)

ax.set_title('Worldwide CO2 Reduction by Category (1990-2020)', fontsize=15, fontweight='bold', pad=15)
ax.set_xlabel('Year', fontsize=11)
ax.set_ylabel('Reduction Volume (Million Tons)', fontsize=11)

ax.grid(color='gray', linestyle='-.', linewidth=0.5, alpha=0.5)
ax.legend(loc='upper right', title='Sectors', fontsize=9, frameon=False)  # Border removed from legend

ax.annotate('Efficiency Boost in Factories', xy=(2005, 150), xytext=(1995, 200),
            arrowprops=dict(facecolor='blue', arrowstyle='->'),
            fontsize=9, bbox=dict(facecolor='lightyellow', edgecolor='blue', boxstyle='round,pad=0.5'))

ax.annotate('Renewables Takeover', xy=(2015, 550), xytext=(2000, 650),
            arrowprops=dict(facecolor='red', arrowstyle='->'),
            fontsize=9, bbox=dict(facecolor='lightgray', edgecolor='red', boxstyle='round,pad=0.5'))
