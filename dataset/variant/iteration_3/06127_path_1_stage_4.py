...
color = '#1f77b4' 
bars1 = ax.barh(x_pos - bar_width, weekday_sleepers, bar_width, label='Weekdays', color=color)
bars2 = ax.barh(x_pos, weekend_sleepers, bar_width, label='Weekends', color=color)
bars3 = ax.barh(x_pos + bar_width, young_adults_sleepers, bar_width, label='Young Adults', color=color)

ax.set_title('Sleep Patterns of Adults', fontsize=14, fontweight='normal', pad=10)
ax.set_ylabel('Hours of Sleep', fontsize=11)
ax.set_xlabel('Number of Individuals', fontsize=11)

ax.set_yticks(x_pos)
ax.set_yticklabels(hours_range, fontsize=9)
...
def add_labels(bars, counts):
    for bar, count in zip(bars, counts):
        ax.text(bar.get_width(), bar.get_y() + bar.get_height() / 2, str(count), va='center', ha='left', fontsize=8)
