...
# Create grouped bar chart for favorite gaming genres
X = np.arange(len(genres))
bar_width = 0.2

ax1.bar(X, alice_genres, width=bar_width, label='Alice', color=single_color)
ax1.bar(X + bar_width, bob_genres, width=bar_width, label='Bob', color=single_color)
ax1.bar(X + 2 * bar_width, charlie_genres, width=bar_width, label='Charlie', color=single_color)
ax1.bar(X + 3 * bar_width, diana_genres, width=bar_width, label='Diana', color=single_color)
...
# Apply single color to the box plot
...
# Adding a legend to box plot manually with a single color
legend_elements = [Patch(facecolor=single_color, label=friend) for friend in friends]
