...
monsoon_wind_speeds = np.array([3.5, 3.3, 3.7, 4.5, 4.1, 3.9, 3.6, 4.0, 4.3])  # New data series

fig, ax = plt.subplots(3, 2, figsize=(14, 15), sharex=True, sharey=True)  # Adjust subplot grid to accommodate new data
seasons = ['Spring', 'Summer', 'Autumn', 'Winter', 'Monsoon']
wind_speed_data = [spring_wind_speeds, summer_wind_speeds, autumn_wind_speeds, winter_wind_speeds, monsoon_wind_speeds]

new_colors = ['purple', 'yellow', 'orange', 'blue', 'green']  # Add a new color
new_markers = ['o', 's', 'D', '^', 'v']  # Add a new marker
new_line_styles = ['-', '--', '-.', ':', '-']  # Use an existing line style

ax = ax.flatten()
for ax_, season, data, color, marker, linestyle in zip(ax, seasons, wind_speed_data, new_colors, new_markers, new_line_styles):
