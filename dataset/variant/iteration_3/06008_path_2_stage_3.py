...
marker_shapes = ['o', 's', 'D']  # Circle, square, and diamond markers
...
for i, flavor in enumerate(sorted_flavors):
    ax.bar(months_idx + i * bar_width, sorted_sales_data[i],
           width=bar_width, label=flavor, alpha=0.7, hatch=marker_shapes[i])
...
ax.grid(True, linestyle=line_styles[1], alpha=0.4)
