...
ax1.plot(months, beta_town_aqi, label='BetaTown', color='blue', marker='s', linestyle='--', linewidth=2)

ax1.fill_between(months, alpha_city_aqi, color='blue', alpha=0.1)
ax1.fill_between(months, beta_town_aqi, color='blue', alpha=0.1)

# Removed textual elements such as title, labels, and annotations
...
# Removing annotation captions
# for city, aqi in [('AlphaCity', alpha_city_aqi), ('BetaTown', beta_town_aqi)]:
#     max_aqi = aqi.max()
#     min_aqi = aqi.min()
...
ax2.bar(cities, improvements, color='blue', alpha=0.5, width=0.4, label='Improvement %')
# Removed textual elements such as labels
