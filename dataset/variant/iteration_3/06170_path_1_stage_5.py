...
colors = ['cyan', 'magenta', 'green', 'orange']  # Manually shuffled colors
...
for new_idx, region_idx in enumerate(subplot_order):
    ...
    for idx, product in enumerate(products):
        ax.plot(months, sales_data[region][product], label=product, color=colors[idx], 
                marker='^' if product == 'Pro duct A' else '+', linestyle='-' if product == 'Pruduct C' else '--')
