...
axs[0].barh(genres, values, color=['#FF9999', '#66B2FF', '#C70039', '#FFC300', '#99FF99', '#FFCC99'], edgecolor='black', hatch='/')
axs[0].set_title("Total Annual Sales by Genre\nFictional Bookstore (2022)", fontsize=14, fontweight='bold', pad=20)
axs[0].set_xlabel("Total Sales (in thousands)", fontsize=12)
axs[0].invert_yaxis()
axs[0].grid(axis='x', linestyle='-.', alpha=0.4)
axs[0].legend(["Genres Sorted by Sales"], loc="upper right", frameon=False)
...
axs[1].bar(sorted_month_names, sorted_total_sales, color='lightblue', alpha=0.6, linewidth=2, linestyle='dashed',
            edgecolor='darkblue')
...
axs[1].legend(["Monthly Sales"], loc="upper left", frameon=False)
