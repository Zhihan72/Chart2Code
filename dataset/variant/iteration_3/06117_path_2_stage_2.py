...
# Snack types and their annual popularity
...
for i, year in enumerate(years):
    ...
    ax.bar(year, popcorn_popularity[i], bottom=chip_popularity[i] + cookie_popularity[i] + fruit_popularity[i] + nut_popularity[i] + candy_popularity[i], width=bar_width, label='Popcorn' if i == 0 else "", color='#ffb3e6', edgecolor='grey')
    ax.bar(year, granola_popularity[i], bottom=chip_popularity[i] + cookie_popularity[i] + fruit_popularity[i] + nut_popularity[i] + candy_popularity[i] + popcorn_popularity[i], width=bar_width, label='Granola' if i == 0 else "", color='#e6e6ff', edgecolor='grey')

# Add annotations for the highest bar in each year
for i, year in enumerate(years):
