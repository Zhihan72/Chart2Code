...
ax1.bar(x_indexes - 1.5*width, apples_sales, width=width, color=bar_color, label='Apples')
ax1.bar(x_indexes - 0.5*width, bananas_sales, width=width, color=bar_color, label='Bananas')
ax1.bar(x_indexes + 0.5*width, oranges_sales, width=width, color=bar_color, label='Oranges')
ax1.bar(x_indexes + 1.5*width, grapes_sales, width=width, color=bar_color, label='Grapes')
...
for i, sales in enumerate(apples_sales):
    ax1.text(i - 1.5*width, apples_sales[i] + 2, f'{apples_sales[i]}', ha='center', va='bottom', fontsize=9, color=bar_color)
for i, sales in enumerate(bananas_sales):
    ax1.text(i - 0.5 * width, bananas_sales[i] + 2, f'{bananas_sales[i]}', ha='center', va='bottom', fontsize=9, color=bar_color)
for i, sales in enumerate(oranges_sales):
    ax1.text(i + 0.5 * width, oranges_sales[i] + 2, f'{oranges_sales[i]}', ha='center', va='bottom', fontsize=9, color=bar_color)
for i, sales in enumerate(grapes_sales):
    ax1.text(i + 1.5 * width, grapes_sales[i] + 2, f'{grapes_sales[i]}', ha='center', va='bottom', fontsize=9, color=bar_color)
