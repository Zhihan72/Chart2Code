...
ax.plot(years, company_a, marker='x', linestyle='-.', color='#33FF57', label='Alpha Corp')
ax.plot(years, company_b, marker='s', linestyle='-', color='#FF5733', label='Beta Inc')
ax.plot(years, company_d, marker='p', linestyle=':', color='#3357FF', label='Delta Group')
ax.plot(years, company_e, marker='h', linestyle='--', color='#FF33A8', label='Epsilon Ltd')
...
ax.legend(title='Corporations', fontsize=12, title_fontsize='13', loc='upper left', borderaxespad=0.5)
...
def annotate_extreme_points(years, prices, ax, color):
    ...
    ax.annotate(f'Peak: {prices[max_idx]} USD', xy=(years[max_idx], prices[max_idx]), xytext=(years[max_idx]+0.5, prices[max_idx]+5),
                arrowprops=dict(facecolor=color, edgecolor='none'), fontsize=9, color=color)
