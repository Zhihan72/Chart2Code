...
# Subplot 1: Grouped bar chart for gardens setup
bar_width = 0.15
indices = np.arange(len(cities))

for i, year in enumerate(years):
    bars = ax1.bar(indices + i * bar_width, gardens_setup[:, i], width=bar_width, label=year)
...
ax1.set_title("Gardens Setup (2019-23)", fontsize=14, fontweight='bold')
...
ax2.set_title("Total Harvest (2019-23)", fontsize=14, fontweight='bold')
ax2.set_xlabel("Cities", fontsize=12)
ax2.set_ylabel("Yield (kg)", fontsize=12)
ax2.yaxis.grid(True, linestyle='--', alpha=0.7)

fig.suptitle("Gardening Initiatives (5 Years)", fontsize=18, fontweight='bold', y=1.02)
plt.show()