...
ax[0].pie(market_share, labels=categories, autopct='%1.1f%%', startangle=90, 
          colors=[single_color]*len(categories), shadow=True, explode=explode, wedgeprops={'edgecolor': 'black'})
...
for category, data in growth_data.items():
    ax[1].plot(years, data, marker='o', label=category, color=single_color)
