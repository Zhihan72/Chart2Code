...
# Subplot 1: Bar chart for gardens setup
bar_width = 0.15
indices = np.arange(len(cities))

for i, year in enumerate(years):
    bars = ax1.bar(indices + i * bar_width, gardens_setup[:, i], width=bar_width, color=consistent_color)
...
bars = ax2.bar(cities, total_harvest_yield, color=consistent_color)
