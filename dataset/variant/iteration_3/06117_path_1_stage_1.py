...
# Use a single consistent color '#8888ff'
uniform_color = '#8888ff'

for i, year in enumerate(years):
    ax.bar(year, chip_popularity[i], width=bar_width, label='Chips' if i == 0 else "", color=uniform_color, edgecolor='grey')
    ax.bar(year, cookie_popularity[i], bottom=chip_popularity[i], width=bar_width, label='Cookies' if i == 0 else "", color=uniform_color, edgecolor='grey')
    ax.bar(year, fruit_popularity[i], bottom=chip_popularity[i] + cookie_popularity[i], width=bar_width, label='Fruits' if i == 0 else "", color=uniform_color, edgecolor='grey')
    ax.bar(year, nut_popularity[i], bottom=chip_popularity[i] + cookie_popularity[i] + fruit_popularity[i], width=bar_width, label='Nuts' if i == 0 else "", color=uniform_color, edgecolor='grey')
    ax.bar(year, candy_popularity[i], bottom=chip_popularity[i] + cookie_popularity[i] + fruit_popularity[i] + nut_popularity[i], width=bar_width, label='Candy' if i == 0 else "", color=uniform_color, edgecolor='grey')
