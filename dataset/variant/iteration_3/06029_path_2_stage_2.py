...
# Stacked Bar Chart for Colony-wise Monthly Consumption (Switching with Histogram)
ax1 = axs[0]
width = 0.3
months_indices = np.arange(len(months))
uniform_color = '#66B3FF'

ax1.bar(months_indices - width, colony_alpha, width, label='Colony Alpha', edgecolor='black', color=uniform_color, alpha=0.7)
ax1.bar(months_indices, colony_beta, width, label='Colony Beta', edgecolor='black', color=uniform_color, alpha=0.7)
ax1.bar(months_indices + width, colony_gamma, width, label='Colony Gamma', edgecolor='black', color=uniform_color, alpha=0.7)
...
# Histogram Plot for Monthly Energy Consumption (Switching with Bar Chart)
ax2 = axs[1]
