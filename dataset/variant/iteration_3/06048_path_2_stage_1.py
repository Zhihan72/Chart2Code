...
nodes = nx.draw_networkx_nodes(G, pos, node_color=node_colors, cmap=colormap, node_size=5000, edgecolors='grey', 
                               node_shape='h')  # Changed node edge color and shape
edges = nx.draw_networkx_edges(G, pos, width=edge_widths, arrowsize=15, edge_color='dimgray', style='dotted')  # Changed edge style
labels = nx.draw_networkx_labels(G, pos, font_size=10, font_weight='bold', font_color='navy')  # Changed label color and size

# Implementing a simple grid, changing the legend frame color and location
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.legend(loc='lower right', title='Influence Level', fontsize=8, frameon=True, framealpha=0.8, edgecolor='black')

plt.title("Influence Network Among Science Fiction Authors", fontsize=16, fontweight='heavy', color='purple')  # Title style change
plt.axis('on')  # Borders are on
