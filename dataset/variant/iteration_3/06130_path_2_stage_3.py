...
# Altered bar chart style with randomized colors and line styles
ax.barh(y_pos, sales_totals, align='center', color='orange', edgecolor='darkblue', linestyle='--')

ax.set_yticks(y_pos)
ax.set_yticklabels(sorted_categories)

ax.set_xlabel('Total Sales (in thousands of units)', fontsize=12, color='darkgreen', fontweight='bold')
ax.set_title('Total Sales for Different Product Categories in 2023', fontsize=14, fontweight='bold', color='purple')

# Altered annotation style with different alignment and font
for i, v in enumerate(sales_totals):
    ax.text(v + 1, i, str(v), va='bottom', fontsize=10, color='red', fontweight='bold')

# Added grid with a different appearance
