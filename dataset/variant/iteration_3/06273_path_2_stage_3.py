...
ax1.legend(handles=legend_elements, title='Friends', loc='upper right', fontsize=10)

X = np.arange(len(genres))
bar_width = 0.25
offset = bar_width * np.arange(len(friends))

ax2.bar(X + offset[0], alice_genres, width=bar_width, label='Alice', color=new_colors[0])  # Changed color
ax2.bar(X + offset[1], bob_genres, width=bar_width, label='Bob', color=new_colors[1])  # Changed color
ax2.bar(X + offset[2], charlie_genres, width=bar_width, label='Charlie', color=new_colors[2])  # Changed color
...
ax2.grid(axis='y', linestyle='--', color='grey', linewidth=0.7)
