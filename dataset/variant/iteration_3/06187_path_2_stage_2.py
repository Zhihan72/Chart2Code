...
colors = ['#32CD32', '#EE82EE', '#FF6347', '#87CEFA', '#FFD700']  # Shuffled colors
for patch, color in zip(box['boxes'], colors):
    patch.set_facecolor(color)

plt.setp(box['medians'], color='darkred', linewidth=3)  # Changed color and width
plt.setp(box['whiskers'], color='black', linewidth=2.5, linestyle='-.')  # Changed color and style
plt.setp(box['caps'], color='black', linewidth=2.5)
plt.setp(box['fliers'], marker='s', color='red', alpha=0.7)  # Changed marker shape and color
...
ax.set_ylabel("Sleep Quality Index", fontsize=14)
ax.grid(axis='x', linestyle=':', alpha=0.5)  # Changed grid axis and style
...
plt.legend(title="Neighborhoods", loc='lower right')  # Changed legend location
