...
linestyles = ['-', ':', '--', '-.', '-', ':', '--', '-.']
markers = ['v', '^', '<', '>', 'o', 'p', 'h', '*']
colors = ['#1f77b4', '#ff7f0e', '#17becf', '#9467bd', 
          '#8c564b', '#2ca02c', '#bcbd22', '#d62728']

for i, year in enumerate(years):
    plt.plot(months, temperature_data[i], marker=markers[i], linestyle=linestyles[i], color=colors[i], label=str(year))

plt.title("Exploring Springfield Temperatures: A Yearly Comparison Over 8 Years", fontsize=16, fontweight='bold')
