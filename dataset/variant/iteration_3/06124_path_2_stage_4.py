...
single_color = '#1f77b4'  # Chosen single consistent color
markers = ['o', '^', 'v', 'x', 's', 'o', 'D', 'p']
linestyles = ['--', '-', '--', '-.', ':', '-', '--', '-.']

for data, name, marker, linestyle in zip(
    [children, teens, twenties, thirties, forties, fifties_plus, seniors, middle_aged], 
    age_groups, markers, linestyles):
    ax.plot(years, data, label=name, color=single_color, marker=marker, linewidth=2, linestyle=linestyle)
    ax.annotate(f'{data[-1]}%', xy=(years[-1], data[-1]), xytext=(5, -5), textcoords='offset points', ha='center', fontsize=9, color=single_color)
