...
new_colors = ['coral', 'royalblue', 'mediumseagreen', 'orange', 'orchid']
markers = ['o', 's', 'd', '^', 'v']
linestyles = ['-', '--', '-.', ':', '-']

for i, (wizard, score) in enumerate(wizards.items()):
    ax.plot(angles, score, linewidth=1.5, linestyle=linestyles[i], label=wizard, color=new_colors[i], marker=markers[i])
    ax.fill(angles, score, color=new_colors[i], alpha=0.1)
...
ax2.barh(list(average_scores.keys()), list(average_scores.values()), color=new_colors, edgecolor='black', hatch='//')
ax2.set_xlim(0, 10)
