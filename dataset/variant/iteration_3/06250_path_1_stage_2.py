...
for ax_, season, data, color in zip(ax.flatten(), seasons, wind_speed_data, new_colors):
    density = gaussian_kde(data)
    x_vals = np.linspace(0, 7, 200)
    y_vals = density(x_vals)

    # Plotting the density horizontally
    ax_.plot(y_vals, x_vals, color=color, label=f'{season}')
