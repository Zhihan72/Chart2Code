...
axs[0].set_title('City C Temps 2050', fontsize=14, fontweight='bold')
axs[0].set_xlabel('Mths', fontsize=12)
axs[0].set_ylabel('Temp (°C)', fontsize=12)
axs[0].set_xticks(np.arange(len(months)))
axs[0].set_xticklabels(months)

season_annot = [('Win', winter_mean), ('Spr', spring_mean), ('Sum', summer_mean), ('Aut', autumn_mean)]
for season, mean_temp in season_annot:
    axs[0].text(len(months)-1, mean_temp + 0.5, f'{season}: {mean_temp:.1f}°C', 
...
axs[1].set_title('City A & B Temps 2050', fontsize=14, fontweight='bold')
axs[1].set_xlabel('Mths', fontsize=12)
axs[1].set_ylabel('Temp (°C)', fontsize=12)
