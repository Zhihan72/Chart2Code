...
# Use a consistent single color for both data groups
common_color = 'tab:green'

ax1.errorbar(years, pm25_concentration, yerr=errors_pm25, label='Air Quality (µg/m³)', fmt='-o', capsize=5, color=common_color, alpha=0.8)
ax1.set_xlabel('Timeline', fontsize=14)
ax1.set_ylabel('Particles in Air (µg/m³)', fontsize=14, color=common_color)
ax1.tick_params(axis='y', labelcolor=common_color)

ax2 = ax1.twinx()
ax2.plot(years, asthma_cases, label='Respiratory Incidents (thousands)', linestyle='--', marker='s', color=common_color, alpha=0.8)
ax2.set_ylabel('Health Cases (thousands)', fontsize=14, color=common_color)
