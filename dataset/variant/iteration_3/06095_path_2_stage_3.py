...
reinforcement_learning = [2, 5, 10, 15, 25, 30, 40, 50, 65, 75, 80]  # New data series

plt.figure(figsize=(12, 8))

plt.plot(years, machine_learning, marker='o', color='purple', linestyle='-', linewidth=2, label='Machine Learning')
plt.plot(years, computer_vision, marker='s', color='orange', linestyle='-', linewidth=2, label='Computer Vision')
plt.plot(years, natural_language_processing, marker='^', color='cyan', linestyle='-', linewidth=2, label='NLP')
plt.plot(years, reinforcement_learning, marker='d', color='green', linestyle='-', linewidth=2, label='Reinforcement Learning')  # New plot for the added series
...
plt.legend()  # Added legend to denote different series
