...
# Alter line style and marker types
markers = ['o', 's', '^', 'D', 'x']  # Added various marker styles
...
for idx, year in enumerate(years):
    ax.bar(bar_positions + idx * bar_width, 
           ...
           color=colors[idx],
           edgecolor='black', 
           hatch=markers[idx % len(markers)])  # Added hatching style

# Randomly shuffle elements by tweaking plot attributes
ax.set_xlabel('High Schools', fontsize=10)  # Adjusted font size
ax.set_ylabel('Production Budget (in thousand USD)', fontsize=10, color='purple')  # Changed text color
...
# Adjust legend properties with different placement and appearance
ax.legend(title='Year', fontsize=11, loc='upper right', frameon=False)

# Change grid settings and border visibility
