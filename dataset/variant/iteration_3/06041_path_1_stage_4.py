...
ax.boxplot(data, patch_artist=True, notch=True, vert=False, widths=0.6,
           boxprops=dict(facecolor='lightblue', color='blue'),
           whiskerprops=dict(color='darkblue'), capprops=dict(color='darkblue'),
           medianprops=dict(color='darkred', linewidth=2))

# Manually shuffled colors
colors = ['lightcoral', 'lightsalmon', 'lightgray', 'lightblue', 'lightgreen', 'lavender', 'lightgoldenrodyellow', 'cyan']
...
for i, city_data in enumerate(data, start=1):
    ...
    ax.scatter(city_data, y_values, alpha=0.6, color='gray', edgecolor='k', label='Individual Data Points' if i == 1 else "")
...
ax.set_yticklabels(cities, rotation=30)
