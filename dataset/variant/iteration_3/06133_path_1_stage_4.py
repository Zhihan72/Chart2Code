...
for idx, store in enumerate(stores):
    ax1.bar(years + idx * bar_width, revenue[idx], width=bar_width, color=color, alpha=0.7)
...
for idx in range(len(stores)):
    ax2.plot(years, satisfaction_scores[idx], marker='o', linestyle='-', linewidth=2, color=color)
    for x, y in zip(years, satisfaction_scores[idx]):
        ax2.annotate(f'{y}', xy=(x, y), xytext=(0, 5), textcoords='offset points', fontsize=9, color=color, ha='center')
