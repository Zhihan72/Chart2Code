...
squarify.plot(sizes=sizes, color=new_colors[:len(sizes)], alpha=0.8, ax=ax1, edgecolor='darkblue', linewidth=1.5)
ax1.axis('off')
...
for i, (platform, growth) in enumerate(growth_trends.items()):
    ax2.plot(years, growth, marker=marker_styles[i % len(marker_styles)], linestyle=line_styles[i % len(line_styles)], markersize=8)
