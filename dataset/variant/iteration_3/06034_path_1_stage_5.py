...
for i, year in enumerate(years):
    axs[0, 0].bar(
        ...
        alpha=0.7,
        color=['chocolate', 'darkorange', 'darkgoldenrod', 'gold', 'goldenrod']
    )
...
axs[1, 0].pie([80, 15, 5], labels=['USA', 'Canada', 'Mexico'], autopct='%1.1f%%', colors=['teal', 'mediumaquamarine', 'lightseagreen'])
axs[1, 0].set_title('NA CO2 Division (2022)', fontsize=14)

# Switch positions of these two pie charts
axs[0, 2].pie([40, 30, 30], labels=['Germany', 'UK', 'France'], autopct='%1.1f%%', colors=['olivedrab', 'darkolivegreen', 'yellowgreen'])
axs[0, 2].set_title('EU CO2 Split (2022)', fontsize=14)

axs[0, 1].pie([50, 30, 20], labels=['China', 'India', 'Japan'], autopct='%1.1f%%', colors=['crimson', 'firebrick', 'lightcoral'])
axs[0, 1].set_title('AS Region CO2 (2022)', fontsize=14)

axs[1, 1].pie([30, 25, 20], labels=['South Africa', 'Nigeria', 'Egypt'], autopct='%1.1f%%', colors=['maroon', 'indianred', 'rosybrown'])
axs[1, 1].set_title('AF Emissions Share (2022)', fontsize=14)

axs[1, 2].pie([50, 30, 20], labels=['Brazil', 'Argentina', 'Colombia'], autopct='%1.1f%%', colors=['plum', 'mediumpurple', 'thistle'])
