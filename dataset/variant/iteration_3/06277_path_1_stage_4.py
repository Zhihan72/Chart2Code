...
for i in range(len(years)):
    axes[0, 0].barh(countries, solar_energy[:, i], label=years[i])
axes[0, 0].set_title('Solar Energy (2017-21)', fontsize=16, fontweight='bold')
axes[0, 0].set_xlabel('Energy (TWh)', fontsize=12)
axes[0, 0].legend(title='Yrs', fontsize=10, loc='upper right', bbox_to_anchor=(1, 1), title_fontsize='12')
axes[0, 0].grid(axis='x', linestyle='--', alpha=0.7)

# Wind Energy
for i in range(len(years)):
    axes[0, 1].barh(countries, wind_energy[:, i], label=years[i])
axes[0, 1].set_title('Wind Energy (2017-21)', fontsize=16, fontweight='bold')
axes[0, 1].set_xlabel('Energy (TWh)', fontsize=12)
axes[0, 1].legend(title='Yrs', fontsize=10, loc='upper right', bbox_to_anchor=(1, 1), title_fontsize='12')
axes[0, 1].grid(axis='x', linestyle='--', alpha=0.7)

# Hydro Energy
for i in range(len(years)):
    axes[1, 0].barh(countries, hydro_energy[:, i], label=years[i])
axes[1, 0].set_title('Hydro Energy (2017-21)', fontsize=16, fontweight='bold')
axes[1, 0].set_xlabel('Energy (TWh)', fontsize=12)
axes[1, 0].legend(title='Yrs', fontsize=10, loc='upper right', bbox_to_anchor=(1, 1), title_fontsize='12')
axes[1, 0].grid(axis='x', linestyle='--', alpha=0.7)

# Geothermal Energy
for i in range(len(years)):
    axes[1, 1].barh(countries, geothermal_energy[:, i], label=years[i])
axes[1, 1].set_title('Geo Energy (2017-21)', fontsize=16, fontweight='bold')
axes[1, 1].set_xlabel('Energy (TWh)', fontsize=12)
axes[1, 1].legend(title='Yrs', fontsize=10, loc='upper right', bbox_to_anchor=(1, 1), title_fontsize='12')
