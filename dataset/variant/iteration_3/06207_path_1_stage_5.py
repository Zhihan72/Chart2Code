...
sizes = [apple_sales.sum(), orange_sales.sum()]
single_color = ['#ff9999']  # One consistent color for all

fig, (ax2, ax1) = plt.subplots(1, 2, figsize=(16, 8))

ax2.pie(sizes, explode=(0.1, 0), labels=['Apple', 'Orange'], colors=single_color * 2,
...
ax1.bar(x - bar_width/2, apple_sales.sum(axis=0), bar_width, label='Apple', color=single_color[0])
ax1.bar(x + bar_width/2, orange_sales.sum(axis=0), bar_width, label='Orange', color=single_color[0])
