...
ax1.bar(X + offset[0], alice_genres, width=bar_width, label='Al', color=new_colors[0])
ax1.bar(X + offset[1], bob_genres, width=bar_width, label='Bob', color=new_colors[1])
ax1.bar(X + offset[2], charlie_genres, width=bar_width, label='Char', color=new_colors[2])

ax1.set_title("Fav Genres\nAmong Friends", fontsize=14, fontweight='bold')
ax1.set_xlabel("Genres", fontsize=12)
ax1.set_ylabel("Pct of Gaming", fontsize=12)
