...
consistent_color = 'Greens'
text_color = 'black'

heatmap = axes[0].imshow(np.ma.masked_array(voltage_levels, mask=~mask), cmap=consistent_color, aspect='auto', interpolation='nearest')
...
for i in range(len(sectors)):
    for j in range(len(months)):
        if i >= j:
            axes[0].text(j, i, f"{voltage_levels[i, j]}", ha='center', va='center', color=text_color, fontsize=7)

# Use the same color for the line plot
axes[1].plot(months, average_voltage, marker='o', color='green', linewidth=2, label='Avg Voltage')
