...
ax1.plot(months, gamma_ville_aqi, label='GammaVille', color='blue', marker='^', linestyle=':', linewidth=2)
...
for city, aqi in [('AlphaCity', alpha_city_aqi),
                  ('BetaTown', beta_town_aqi),
                  ('GammaVille', gamma_ville_aqi)]:
    max_aqi = aqi.max()
    min_aqi = aqi.min()
