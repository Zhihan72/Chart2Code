...
def plot_radar(ax, data, colors):
    for idx, d in enumerate(data):
        ...
        ax.fill(angles, values, color=colors[idx], alpha=0.4)  # Increased alpha for stronger fill
...
ax.set_yticklabels([])  # Removing value labels on y-axis for cleaner fill area focus
ax.set_ylim(0, 10)
plt.grid(False)  # Remove grids for improved fill area emphasis
