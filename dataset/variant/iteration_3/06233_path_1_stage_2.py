...
# Adding random markers and line style to the stackplot elements
markers_list = ['o', '^', 's', 'd', 'x']
line_styles = ['-', '--', '-.', ':', '-']
for data, marker, line in zip(sustainability_efforts, markers_list, line_styles):
    ax.plot(years, np.cumsum(data), marker=marker, linestyle=line, alpha=0.5)
