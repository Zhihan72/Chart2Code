...
# Shortened bird species names
bird_species = ['Stk', 'Egl', 'Her', 'Flm', 'Spr', 'Hwk', 'Pln', 'Swn']
...
for i, altitude in enumerate(data):
    sns.kdeplot(altitude, ax=ax[0], label=bird_species[i], color=single_color, shade=True, alpha=0.6)

# Shortened titles and labels
ax[0].set_title('Altitude Density', fontsize=16, fontweight='bold', pad=20)
ax[0].set_xlabel('Altitude (m)', fontsize=12)
...
ax[1].set_title('Altitude Distribution', fontsize=16, fontweight='bold', pad=20)
ax[1].set_xlabel('Altitude (m)', fontsize=12)
ax[1].set_ylabel('Species', fontsize=12)
