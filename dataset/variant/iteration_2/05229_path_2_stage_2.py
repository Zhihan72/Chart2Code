...
box = ax.boxplot(noise_data, vert=False, patch_artist=True, labels=locations, notch=True, whis=1.5)
...
for i, (mean, std_dev) in enumerate(zip(means, std_devs)):
    ax.text(mean + 1, i + 1, f"μ={mean:.1f}\nσ={std_dev:.1f}", ha='left', va='center', fontsize=9, color='black')
...
ax.set_ylabel("City Locations", fontsize=13)
ax.set_xlabel("Noise Levels (dB)", fontsize=13)
ax.xaxis.grid(True, linestyle='--', alpha=0.7)

acceptable_noise_level = 65
ax.axvline(acceptable_noise_level, color='r', linestyle='--', linewidth=1.5, label='Acceptable Noise Level (65 dB)')
