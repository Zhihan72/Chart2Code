...
# Plotting each genre
for i, (genre_data, color, label) in enumerate(
    zip([action, comedy, drama, sci_fi, animation], new_colors,
        ['Funny', 'Exciting', 'Emotional', 'Sci-Fi', 'Animated'])):
    ax.bar(years + i * bar_width, genre_data, width=bar_width, label=label, color=color, alpha=0.8, edgecolor='black', linewidth=1.5)

# Settings for the chart's elements
...
# Customizing x-ticks
ax.set_xticks(years + bar_width * 2, labels=years)
