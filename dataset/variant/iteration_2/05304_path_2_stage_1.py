...
# Mask the lower triangle of the data
mask = np.triu(np.ones_like(sales_data, dtype=bool)) # Upper triangle mask

# Plot the main heatmap for regional sales data
cax = ax1.matshow(np.where(mask, np.nan, sales_data), cmap='viridis', aspect='auto')
