...
# Define the edges with weights for an undirected graph
edges = [
    ('Earth', 'Mars', 300), 
    ('Earth', 'Jupiter', 500), 
    ('Earth', 'Venus', 200), 
...
# Define a node color map for visualization
new_node_color_map = ['lightgreen', 'lightblue', 'orange', 'pink', 'lightgray', 'lightyellow', 'lightcyan', 'beige', 'lightpink']
nx.draw_networkx_nodes(G, pos, node_size=1200, node_color=new_node_color_map, alpha=0.9, edgecolors='black')

# Define edge width based on weight
edge_width = [0.05 * G[u][v]['weight'] for u, v in G.edges]
nx.draw_networkx_edges(G, pos, width=edge_width, edge_color='gray', alpha=0.8)

# Plot scatter points to indicate economic significance
