...
marker_styles = ['s', '^', 'x', 'd']
line_styles = ['-', '--', '-.', '-']
colors = ['green', 'blue', 'magenta', 'orange'] # Manually assigned colors

for i, country in enumerate(countries):
    plt.plot(years, co2_reductions[i], marker=marker_styles[i], linestyle=line_styles[i], color=colors[i], label=country)
