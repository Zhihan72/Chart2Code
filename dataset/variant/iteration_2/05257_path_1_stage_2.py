...
# Randomly altering the style of each bar for visual differentiation
bar_colors = ['#FF9999', '#66B3FF', '#99FF99', '#FFCC99', '#CCCCFF']
hatch_styles = ['/', '\\', '|', '-', '+']

for idx, (energy, energy_type, color, hatch) in enumerate(zip(energy_data, energy_types, bar_colors, hatch_styles)):
    ax.bar(x_pos + idx * bar_width, energy, bar_width, label=energy_type, color=color, hatch=hatch, edgecolor='black')

ax.set_xlabel('Year', fontsize=12)
ax.set_ylabel('Consumption (GWh)', fontsize=12)
ax.set_title('Growth in Renewable Energy Consumption (2010 - 2020)', fontsize=18, fontweight='bold')
ax.set_xticks(x_pos + bar_width * (len(energy_types) - 1) / 2)
ax.set_xticklabels(years, rotation=45)
ax.legend(title='Energy Source', fontsize=10, loc='upper left')

# Adding grid with different style
ax.yaxis.grid(True, linestyle='-.', linewidth=0.5, alpha=0.8)
ax.xaxis.grid(True, linestyle=':', linewidth=0.5, alpha=0.3)

# Adding border alteration
...
for idx, energy in enumerate(energy_data):
    for pos, val in enumerate(energy):
        ax.text(pos + idx * bar_width, val + 10, f'{val}', ha='center', va='bottom', fontsize=8, rotation=90)
