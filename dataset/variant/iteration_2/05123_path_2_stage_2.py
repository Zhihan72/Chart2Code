...
ax.plot(medians, range(1, len(age_groups) + 1), color='purple', marker='D', linestyle='-.', label='Median Score')
...
for i, age_group in enumerate(age_groups):
    ax.text(max(happiness_data[i]) + 0.5, i + 1, f'Max: {max(happiness_data[i])}', 
