...
# Horizontal box plot in the first position with overlay of individual data points and shuffled colors
axs[0].boxplot(box_plot_data, vert=False, patch_artist=True, notch=True, 
...
for i, data in enumerate(box_plot_data):
    y = [i + 1] * len(data)
    axs[0].scatter(data, y, alpha=0.6, color=shuffled_colors[i], edgecolor='darkblue', linewidth=0.7)

axs[0].set_yticks(np.arange(1, len(labels) + 1))
axs[0].set_yticklabels(labels, fontsize=12)

# Horizontal violin plot in the second position with shuffled colors
