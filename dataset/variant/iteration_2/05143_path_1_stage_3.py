...
soybean_yield = [-2.0, -2.1, -2.2, -2.3, -2.4]  # Inverted to negative
rice_yield = [-4.2, -4.3, -4.5, -4.6, -4.5]  # Inverted to negative
sunflower_yield = [-1.8, -1.9, -2.0, -2.1, -2.2]  # Inverted to negative
...
# Negative yields
ax.bar(years, soybean_yield, label='Soybeans', color=colors[3], hatch='xx')
ax.bar(years, rice_yield, bottom=soybean_yield, label='Rice', color=colors[4], hatch='//')
ax.bar(years, sunflower_yield, bottom=np.array(soybean_yield) + np.array(rice_yield), label='Sunflower', color=colors[5], hatch='\\\\')
