...
for bar in bars:
    yval = bar.get_height()
    ax1.text(bar.get_x() + bar.get_width() / 2, yval + 2, f'{yval} mg', ha='center', va='bottom', color='darkred', fontsize=11)
...
ax2 = ax1.twinx()
ax2.plot(fruits, avg_rating, color="purple", marker='s', linestyle='--', linewidth=3, markersize=8, label='Taste Score (out of 10)')

# Labels for ratings
for i, rating in enumerate(avg_rating):
    ax2.text(i, rating + 0.2, f'{rating}', ha='center', va='bottom', color='purple', fontsize=10)

ax2.set_ylabel('Taste Score Board', fontsize=14, color='purple')
