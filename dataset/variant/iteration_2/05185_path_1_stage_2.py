...
for i in range(len(age_groups)):
    axes[1].bar(indices + i * bar_width, data[i], bar_width, label=age_groups[i], color=colors[i])

axes[1].set_title('Activity Choices by Age', fontsize=16, fontweight='bold')
axes[1].set_xlabel('Leisure Activities', fontsize=13)
axes[1].set_ylabel('Engagement Percentage', fontsize=13)
axes[1].set_xticks(indices + bar_width * (len(age_groups) / 2 - 0.5))
axes[1].set_xticklabels(hobbies, fontsize=12)
axes[1].legend(title='Demographics', fontsize=11)
