...
# Shuffled colors
colors = ['#1f77b4', '#ff7f0e', '#d62728', '#2ca02c']  # Previously, it was ['#d62728', '#2ca02c', '#1f77b4', '#ff7f0e']
labels = ['Bio', 'Hydro', 'Wind', 'Solar']

ax.stackplot(years, energy_data, labels=labels, colors=colors, alpha=0.9)
ax.set_title('Renewables (2000-20)', fontsize=18, fontweight='bold', ha='center')
ax.set_xlabel('Yr', fontsize=14)
ax.set_ylabel('Share (%)', fontsize=14)
ax.legend(loc='upper right', fontsize=12, title='Sources', frameon=False)
...
for source, year in significant_years.items():
    ax.annotate(f"{source[:3]}: {annotations[source]}%",
                xy=(year, annotations[source]), 
                xytext=(year - 5, annotations[source] + 5),
