...
# Calculate the average sales for each fruit
...
# Plot each fruit with its sales deviation from the average
bar1_apples = ax.bar(positions, apples_sales - average_apples, width=bar_width, color='red', label='Apples', edgecolor='black', alpha=0.8)
bar2_bananas = ax.bar(positions, bananas_sales - average_bananas, width=bar_width, bottom=apples_sales - average_apples, color='yellow', label='Bananas', edgecolor='black', alpha=0.8)
bar3_oranges = ax.bar(positions, oranges_sales - average_oranges, width=bar_width, bottom=(apples_sales + bananas_sales) - (average_apples + average_bananas), color='orange', label='Oranges', edgecolor='black', alpha=0.8)
...
ax.axhline(0, color='black', linewidth=0.8)  # Central axis for diverging chart
