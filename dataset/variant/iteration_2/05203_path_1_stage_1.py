...
# Plot the total emissions as a horizontal bar chart (originally axs[1] now switched to axs[0])
...
# Create the bar chart for each month (originally axs[0] now switched to axs[1])
for i, city in enumerate(cities):
    axs[1].bar(np.arange(len(months)) + i * 0.15, emissions_data[i], width=0.15, color=colors[i], label=city)
