...
ax.annotate('Growth steadies midpoint', xy=(26, species_2_growth[25]), xytext=(30, 34),
            arrowprops=dict(facecolor='black', arrowstyle='->'), fontsize=9, fontweight='bold')

for i, week in enumerate(weeks):
    if i % 10 == 0:
        ax.text(week, species_1_growth[i] + 0.5, f'Plant A: {species_1_growth[i]:.1f}', fontsize=8, ha='center', va='bottom')
        ax.text(week, species_2_growth[i] + 0.5, f'Species B: {species_2_growth[i]:.1f}', fontsize=8, ha='center', va='bottom')
