...
plt.plot(years, europe, marker='d', linestyle='-', color='cyan', label='Eur', linewidth=2)
plt.plot(years, north_america, marker='v', linestyle=':', color='magenta', label='NA', linewidth=2)

plt.title("Solar Energy Growth (2010-2020)", fontsize=18, fontweight='bold', pad=20)
...
plt.legend(title='Reg', loc='lower right', fontsize=12, shadow=True)

for region, data, color in zip(['Asia', 'Eur', 'NA'],
                               [asia, europe, north_america],
                               ['red', 'cyan', 'magenta']):
    max_gen = max(data)
    max_year = years[data.index(max_gen)]
    plt.annotate(f'{max_gen} GW', xy=(max_year, max_gen),
