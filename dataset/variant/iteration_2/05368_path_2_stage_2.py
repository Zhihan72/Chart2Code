...
# Define a consistent color for all institutions
single_color = ['#66b3ff']  # Choose one color
...
def plot_pie_chart(data, institution, categories, color):
    ...
    wedges, texts, autotexts = ax.pie(
        ...
        startangle=140,
        colors=color * len(categories),  # Apply the single color to all categories
        pctdistance=0.8,
...
for institution, data in data_by_institution.items():
    plot_pie_chart(data, institution, categories, single_color)