...
ax1.plot(years, renewable_energy, label='Renewable Energy Usage', color='green', linewidth=2, marker='s', linestyle='-', markersize=6)
ax1.set_title('CO2 Emissions and Renewable Energy Usage Over Two Decades', fontsize=16, fontweight='bold', pad=20)
ax1.set_ylabel('Renewable Energy (Billion kWh)', fontsize=12)
ax1.grid(True, linestyle='--', alpha=0.5)
ax1.legend(loc='upper left', fontsize=10)

# Plot CO2 emissions as a line chart
ax2.plot(years, co2_emissions, label='CO2 Emissions', color='red', linewidth=2, marker='o', linestyle='-', markersize=6)
ax2.set_xlabel('Year', fontsize=12)
ax2.set_ylabel('CO2 Emissions (Million Metric Tons)', fontsize=12)
ax2.grid(True, linestyle='--', alpha=0.5)
ax2.legend(loc='upper right', fontsize=10)

# Annotate the significant events and data points on the charts
ax1.annotate('Significant Rise in Renewable\nEnergy Usage Starts', xy=(2012, 50), xytext=(2006, 110),
             arrowprops=dict(facecolor='green', arrowstyle='->', lw=1.5), fontsize=10, color='green')

ax2.annotate('Peak CO2 Emissions', xy=(2010, 340), xytext=(2005, 350),
