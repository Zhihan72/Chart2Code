...
wing_spans_tropical = [1.2, 1.0, 0.3, 1.8, 2.5]
wing_spans_temperate = [1.1, 3.2, 0.35, 2.0, 0.9]
flying_speeds_tropical = [55, 24, 30, 50, 60]
flying_speeds_temperate = [53, 65, 33, 47, 20]
...
for i, sp in enumerate(species):
    axj.scatter(wing_spans_temperate[i], flying_speeds_temperate[i], 
                color=single_color, label=f'{sp} in Cool', s=100, marker=markers[i])
...
for i, sp in enumerate(species):
    axi.scatter(wing_spans_tropical[i], flying_speeds_tropical[i], 
                color=single_color, label=f'{sp} in Hot', s=100, marker=markers[i])
