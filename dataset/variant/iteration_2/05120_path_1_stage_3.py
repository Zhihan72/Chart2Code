...
# Changed color for all data groups to a single consistent color
color = 'slateblue'
ax.barh(y_pos - 1.5*width, social_media, height=width, color=color)
ax.barh(y_pos - 0.5*width, gaming, height=width, color=color)
ax.barh(y_pos + 0.5*width, productivity, height=width, color=color)
ax.barh(y_pos + 1.5*width, entertainment, height=width, color=color)
