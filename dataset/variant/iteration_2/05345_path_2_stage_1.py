...
# Randomly chosen marker type and line style for each variety
marker_styles = ['o', 's', 'D', '^']
line_styles = ['-', '--', '-.', ':']

# 1st subplot
for idx, variety in enumerate(lettuce_varieties):
    axs[0].plot(months, sales_data[idx], color=colors[idx], marker=marker_styles[idx], linestyle=line_styles[idx], alpha=0.7, label=variety)
