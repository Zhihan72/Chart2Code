...
for i, city in enumerate(cities):
    axs[0].plot(months, rainfall_data[i], marker=marker, color=color, label=city)

axs[0].set_xlabel('Months', fontsize=12)
axs[0].set_ylabel('Average Monthly Rainfall (mm)', fontsize=12)
axs[0].set_title('Yearly Rainfall Trend by City (2023)', fontsize=14, fontweight='bold')
axs[0].legend(loc='best')

# Horizontal Bar plot
for i, city in enumerate(cities):
    bars = axs[1].barh(y + i * bar_width, rainfall_data[i], bar_width, label=city, color=color)
    for bar in bars:
        axs[1].annotate(f'{bar.get_width()}',
                        xy=(bar.get_width(), bar.get_y() + bar.get_height() / 2),
                        xytext=(3, 0), textcoords='offset points',
                        ha='left', va='center', fontsize=9)

axs[1].set_ylabel('Months', fontsize=12)
axs[1].set_xlabel('Average Monthly Rainfall (mm)', fontsize=12)
axs[1].set_title('Rainfall Land: Average Monthly Rainfall by City (2023)', fontsize=14, fontweight='bold')
axs[1].set_yticks(y + bar_width * (len(cities) / 2 - 0.5))
axs[1].set_yticklabels(months, fontsize=10)
