...
# Sorted bar chart
bars1 = ax1.bar(years_sorted - 0.2, startups_founded_sorted, width=0.4, label='Region 1', color=plt.cm.tab20c(0), edgecolor='black')
bars2 = ax1.bar(years_sorted_region2 + 0.2, startups_founded_region2_sorted, width=0.4, label='Region 2', color=plt.cm.tab20c(4), edgecolor='black')
...
for bar, value in zip(bars1, startups_founded_sorted):
    ax1.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 3, f'{value}', ha='center', va='bottom', fontsize=12)
for bar, value in zip(bars2, startups_founded_region2_sorted):
    ax1.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 3, f'{value}', ha='center', va='bottom', fontsize=12)

# Line chart with additional data series
