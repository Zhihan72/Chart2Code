...
# Plotting the bar chart for growth rates on the left
x = np.arange(len(years))
width = 0.25

ax1.bar(x[1:] - width, north_america_growth, width, label='North America', color='skyblue')
ax1.bar(x[1:], europe_growth, width, label='Europe', color='lightgreen')
ax1.bar(x[1:] + width, asia_growth, width, label='Asia', color='lightcoral')
...
# Plotting the bar chart for average books read on the right
ax2.bar(x - width, north_america_books, width, label='North America', color='skyblue')
ax2.bar(x, europe_books, width, label='Europe', color='lightgreen')
ax2.bar(x + width, asia_books, width, label='Asia', color='lightcoral')
...
# Adding text annotations to highlight significant changes in the average books plot
ax2.annotate('Steady Growth', xy=(8, 8.7), xytext=(6, 9.5),
             arrowprops=dict(facecolor='black', arrowstyle='->', lw=1), fontsize=10, color='navy', weight='bold')
ax2.annotate('Rapid Improvement', xy=(8, 5.8), xytext=(6, 6.5),
