...
axes[0].set_title('Wheat Yields in 2022', fontsize=14, fontstyle='italic')
axes[1].set_title('Rice Yields in 2022', fontsize=14, fontweight='light')
axes[2].set_title('Corn Yields in 2022', fontsize=14, fontweight='bold')

box_wheat = axes[0].boxplot(data_wheat, vert=False, patch_artist=True, labels=regions, notch=False)
for patch in box_wheat['boxes']:
    patch.set_facecolor('#D48B91')
axes[0].set_xlabel('Yield (tons per hectare)', fontsize=12, color='darkred')

box_rice = axes[1].boxplot(data_rice, vert=False, patch_artist=True, labels=regions, notch=True)
for patch in box_rice['boxes']:
    patch.set_facecolor('#7EA2DD')
axes[1].set_xlabel('Yield (tons per hectare)', fontsize=12, color='navy')
...
for patch in box_corn['boxes']:
    patch.set_facecolor('#A1D99B')
axes[2].set_xlabel('Yield (tons per hectare)', fontsize=12, color='green')
...
fig.suptitle('Climate Change Impact on Crop Yields in 2022', fontsize=18, fontweight='heavy', style='italic', y=1.02)
