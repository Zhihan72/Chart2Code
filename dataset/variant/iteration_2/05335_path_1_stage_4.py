...
plt.plot(years, ev_korea, color='purple', marker='p', linestyle='--', linewidth=2, markersize=5, label='Korea')
plt.plot(years, ev_australia, color='cyan', marker='d', linestyle='-', linewidth=2, markersize=4, label='Aus')

plt.title('EV Adoption (2015-2025)', fontsize=16, fontweight='bold', pad=20)
plt.xlabel('Year', fontsize=14)
plt.ylabel('EV Rate (M)', fontsize=14)

plt.grid(linestyle='-', alpha=0.7)
plt.legend(title='Regions', fontsize=10, loc='lower right', frameon=False)

annotations = [
    ('Rapid Growth', 2020, 10.0),
    ('Kickstart', 2020, 3.0),
    ('Accel. Growth', 2021, 6.5)
]

for text, x, y in annotations:
    plt.annotate(text, xy=(x, y), xytext=(-30, 20), textcoords='offset points',
                 arrowprops=dict(arrowstyle='->', color='black'), fontsize=11, color='grey')
