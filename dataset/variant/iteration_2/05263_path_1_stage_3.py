...
# Sorted bar chart data (e.g., descending on each technology's adoption rate)
sorted_indices = np.argsort(adoption_data, axis=1)
sorted_data = np.sort(adoption_data, axis=1)  # Sort the data in ascending order

# Update regions order based on sorting
sorted_regions = [[regions[i] for i in sorted_indices[j]] for j in range(len(sorted_indices))]

width = 0.2
colors = ['#ff7f0e', '#d62728', '#2ca02c', '#1f77b4']

# Create separate bar plots for each sorted data
