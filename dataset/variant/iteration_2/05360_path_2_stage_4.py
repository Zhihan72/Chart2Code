...
# Randomized stylistic elements and altered original settings
...
ax2.plot(years, cumulative_A, marker='v', linestyle='--', color='blue', label='Accumulated A', linewidth=1.5)
ax2.plot(years, cumulative_B, marker='x', linestyle='-.', color='magenta', label='Total B', linewidth=1.5)
ax2.plot(years, cumulative_C, marker='p', linestyle='-', color='cyan', label='Sum C', linewidth=1.5)
ax2.plot(years, cumulative_D, marker='h', linestyle='--', color='lime', label='Compiled D', linewidth=1.5)
ax2.plot(years, cumulative_E, marker='D', linestyle=':', color='brown', label='Aggregate E', linewidth=1.5)
...
ax2.legend(title='Total Groups', loc='lower right', fontsize=10, title_fontsize=11)
ax2.grid(False)  # Removed grid to show variety

for year, value in zip(years, cumulative_A):
    ax2.annotate(f'{value}', xy=(year, value), xytext=(0, 10), textcoords='offset points', ha='center', fontsize=9, color='blue')
for year, value in zip(years, cumulative_B):
    ax2.annotate(f'{value}', xy=(year, value), xytext=(0, 10), textcoords='offset points', ha='center', fontsize=9, color='magenta')
for year, value in zip(years, cumulative_C):
    ax2.annotate(f'{value}', xy=(year, value), xytext=(0, 10), textcoords='offset points', ha='center', fontsize=9, color='cyan')
for year, value in zip(years, cumulative_D):
    ax2.annotate(f'{value}', xy=(year, value), xytext=(0, 10), textcoords='offset points', ha='center', fontsize=9, color='lime')
for year, value in zip(years, cumulative_E):
    ax2.annotate(f'{value}', xy=(year, value), xytext=(0, 10), textcoords='offset points', ha='center', fontsize=9, color='brown')

# Changed border style of plots
for axis in [ax1, ax2]:
