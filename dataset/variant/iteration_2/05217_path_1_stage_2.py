...
ax1.hist([child_sleep, teen_sleep, adult_sleep, senior_sleep], 
         bins=bins, label=age_groups, rwidth=0.9, color=[single_color for _ in range(4)])
...
for i, data in enumerate([child_sleep, teen_sleep, adult_sleep, senior_sleep]):
    ax2.hist(data, bins=bins, alpha=0.6, label=age_groups[i], color=single_color, edgecolor='black')
