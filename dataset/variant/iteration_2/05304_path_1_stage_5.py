...
cax = ax1.matshow(masked_data, cmap='plasma', aspect='auto')  # Changed colormap for stylistic variation
ax1.set_xticks(np.arange(len(categories)))
ax1.set_yticks(np.arange(len(regions)))
ax1.set_xticklabels(categories, rotation=45, ha='right')  # Altered alignment
ax1.set_yticklabels(regions)
fig.colorbar(cax, ax=ax1, orientation='vertical', fraction=0.046, pad=0.04)
ax1.set_title('Triangular Sales by Region/Category', pad=25, fontstyle='italic')  # Changed title padding and font style

# Removed colored text to reduce clutter in dense areas
for i in range(masked_data.shape[0]):
    for j in range(masked_data.shape[1]):
        if masked_data[i, j] != 0:
            ax1.text(j, i, f"{masked_data[i, j]:.0f}", ha='center', va='center', color='black')

months = np.arange(1, 13)
line_styles = ['-', '--', '-.', ':', '-', '--']  # Altered line styles for variation
markers = ['o', '^', 's', 'p', '*', 'D']  # Changed marker styles

for i in range(trend_data.shape[0]):
    ax2.plot(months, trend_data[i], label=categories[i], linestyle=line_styles[i], marker=markers[i])

ax2.set_title('Product Sales Trends', pad=15)
ax2.set_xlabel('Month')
ax2.set_ylabel('Avg Sales')
ax2.legend(loc='upper left', bbox_to_anchor=(1, 0.5))  # Adjusted legend location
