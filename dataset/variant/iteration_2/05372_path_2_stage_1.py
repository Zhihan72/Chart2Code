...
# Use varied marker types and line styles for stylistic changes
colors = ['#8e44ad', '#3498db', '#e74c3c', '#2ecc71']
line_styles = ['-', '--', '-.', ':']

for i, (sales, color, line_style) in enumerate(zip(data, colors, line_styles)):
    rects = ax.bar(x + i * width, sales, width, 
                   label=f'Q{i+1}', color=color, linestyle=line_style)
    for rect in rects:
