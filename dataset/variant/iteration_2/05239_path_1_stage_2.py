...
fig, (ax2, ax1) = plt.subplots(1, 2, figsize=(14, 7))  # Switch ax1 and ax2
...
bars2 = ax2.bar(x_pos + width/2, economic_impact, width, color='#8B4513', edgecolor='black', linewidth=1.2, label='Economic Impact (B USD)')
for bar in bars2:
    yval = bar.get_height()
...
bars1 = ax1.bar(x_pos - width/2, coffee_consumption, width, color='#D2691E', edgecolor='black', linewidth=1.2, label='Consumption (kg/person)')
for bar in bars1:
    yval = bar.get_height()
