...
# Manually altered colors and line styles.
colors = ['#FF5733', '#33FF57', '#FF33FF', '#3357FF', '#FFD733']
line_styles = ['-', '--', '-.', ':', '-']
...
for i, ax in enumerate(axes):
    ...
    for j in range(len(companies)):
        ...
        ax.plot(x + j * 0.15, year_data[j], linestyle=line_styles[j%len(line_styles)], marker='o', color=colors[j])
...
axes[0].legend(loc='lower left', fontsize=10, title='Firm Names')
