...
# Shuffled colors without using random functions
shuffled_colors = ['#F0DB4F', '#00599C', '#3776AB', '#B07219'] 
line_styles = ['-', '--', '-.', ':']

fig, ax1 = plt.subplots(figsize=(14, 8))

bars = ax1.barh(languages, popularity, color=shuffled_colors, edgecolor='grey', height=0.6)
...
for idx, (language, growth) in enumerate(projected_jobs_growth.items()):
    ax2.plot(growth, years, label=f'Projected {language}', color=shuffled_colors[idx], linestyle=line_styles[idx], marker='o')
