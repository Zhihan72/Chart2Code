...
# Initialize a graph for each temperature zone
G_polar = nx.Graph()
...
# Draw graphs on the same plot with different colors for each zone
nx.draw(G_polar, pos=nx.spring_layout(G_polar), node_color='b', edge_color='b', with_labels=True, ax=ax, label='Polar')
nx.draw(G_temperate, pos=nx.spring_layout(G_tropical), node_color='g', edge_color='g', with_labels=True, ax=ax, label='Temperate')
