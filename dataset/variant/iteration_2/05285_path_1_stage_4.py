...
# Change marker and line styles for variety
ax.plot(hours, cpu_usage, label='CPU', color='coral', marker='*', linestyle='-.', linewidth=2.5)
ax.plot(hours, ram_usage, label='RAM', color='lime', marker='x', linestyle='-', linewidth=2.5)
ax.plot(hours, disk_usage, label='Disk', color='navy', marker='v', linestyle=':', linewidth=2.5)
ax.plot(hours, network_usage, label='Net', color='orchid', marker='+', linestyle='--', linewidth=2.5)

ax.set_title("System Perf. (CPU, RAM, Disk, Net)", fontsize=18, fontweight='bold', pad=20)
ax.set_xlabel("Hour", fontsize=14)
ax.set_ylabel("Usage (%)", fontsize=14)

# Change grid style and add grid lines
ax.grid(True, linestyle='-', alpha=0.3)

# Change legend location and font size for variety
ax.legend(loc='lower left', fontsize=10)

# Annotations for peaks and lows
...
# More space to accommodate changes
plt.tight_layout()
plt.show()