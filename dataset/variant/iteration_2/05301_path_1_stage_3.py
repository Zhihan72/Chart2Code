...
average_heights = [np.mean(xenia_height), np.mean(zog_height), np.mean(trogar_height), np.mean(vornas_height)]
average_diameters = [np.mean(xenia_diameter), np.mean(zog_diameter), np.mean(trogar_diameter), np.mean(vornas_diameter)]
...
ax2.set_xticklabels(['Gal X', 'Terr Z', 'Planet T', 'Realm V'])
ax2.legend()
