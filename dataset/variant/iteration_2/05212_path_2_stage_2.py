...
# Randomly shuffled colors for each country
...
# Randomly shuffled location and font size of the legend
ax.legend(title="Country", fontsize=9, loc='lower center')

# Added grid with different line style
ax.yaxis.grid(True, linestyle='-.', linewidth=1.0, alpha=0.7)

# Add borders by altering spine parameters
for spine in ax.spines.values():
    spine.set_edgecolor('#2ca02c')
    spine.set_linewidth(1.5)

# Randomized marker shapes and line styles
markers = ['o', '^', 's', 'D', 'x', '*']
linestyles = ['-', '--', '-.', ':']

for i, (country_data, marker) in enumerate(zip(data, markers)):
    ax.plot(x + i * width, country_data, marker=marker, linestyle=np.random.choice(linestyles), color=colors[i])
