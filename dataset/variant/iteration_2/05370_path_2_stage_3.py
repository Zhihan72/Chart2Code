...
ax.stackplot(years, burnt_area_data, labels=['Savanna', 'Mixed', 'Deciduous', 'Coniferous', 'Tropical'], colors=[single_color]*5, alpha=0.8)
...
for year, value, label in zip(significant_years, significant_values, forest_labels):
    ax.annotate(f'{value}k Ha', xy=(year, value), xytext=(-30, 10),
                textcoords='offset points', fontsize=10,
                arrowprops=dict(facecolor=single_color, arrowstyle="wedge", alpha=0.5),
                bbox=dict(facecolor='white', edgecolor=single_color, boxstyle='round,pad=0.3'))

custom_legend = [plt.Line2D([0], [0], color=single_color, marker=marker, lw=2) for marker in marker_styles]
