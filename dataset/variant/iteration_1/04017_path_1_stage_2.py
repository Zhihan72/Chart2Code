...
for idx, region in enumerate(regions):
    ax[idx].barh(years - height, solar_projects[region], height=height, label='Solar Projects', color='darkorange', hatch='//')
    ax[idx].barh(years, wind_projects[region], height=height, label='Wind Projects', color='deepskyblue', hatch='x')
    ax[idx].barh(years + height, hydro_projects[region], height=height, label='Hydro Projects', color='darkseagreen', hatch='o')
    
    ax[idx].set_title(f'{region} - Green Energy Projects', fontsize=14, fontweight='bold', pad=15)
    ax[idx].set_ylabel('Year', fontsize=12)
    ax[idx].set_xlabel('Projects', fontsize=12)
