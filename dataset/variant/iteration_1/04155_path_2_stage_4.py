...
plt.plot(years, solar_energy, color='#FF6347', linestyle='--', linewidth=1.5, marker='x', label='Solar Energy')
plt.plot(years, wind_energy, color='#8A2BE2', linestyle='-.', linewidth=2.5, marker='p', label='Wind Energy')
plt.plot(years, hydro_energy, color='#3CB371', linestyle=':', linewidth=2, marker='2', label='Hydro Energy')
plt.plot(years, geothermal_energy, color='#CD5C5C', linestyle='-', linewidth=3, marker='v', label='Geothermal Energy')
plt.plot(years, biomass_energy, color='#FF4500', linestyle='-', linewidth=2, marker='h', label='Biomass Energy')
plt.plot(years, wave_energy, color='#BA55D3', linestyle='-', linewidth=1, marker='_', label='Wave Energy')

plt.grid(False)  # Removed the grid
plt.xticks(years[::3], rotation=30)  # Changed ticks interval and rotation
plt.yticks(np.arange(0, 305, 30))  # Changed intervals for y-ticks
plt.legend(loc='upper right', fontsize=12, shadow=True)  # Moved and styled legend
plt.tight_layout()
plt.show()