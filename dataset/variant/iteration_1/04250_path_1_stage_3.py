...
for sales, label, linestyle, marker in zip(
    [model_alpha_sales, model_beta_sales, model_gamma_sales, model_delta_sales],
    labels, linestyles, markers):

    ax.plot(months, sales, marker=marker, label=label, linestyle=linestyle, linewidth=2, color=common_color)

for sales, label in zip(
    [model_alpha_sales, model_beta_sales, model_gamma_sales, model_delta_sales],
    labels):
    ...
    ax.annotate(
        ...
        arrowprops=dict(facecolor=common_color, shrink=0.05),
        fontsize=10, color=common_color, ha='center'
    )
