...
plt.title("Robot Arm Performance (Shuffled Styling)", fontsize=18, fontweight='bold')
plt.xlabel("Speed", fontsize=14)
plt.ylabel("Items Processed", fontsize=14)

cbar = plt.colorbar(sc)
cbar.set_label('Performance (Items/Speed)', fontsize=12)

for i, txt in enumerate(robot_ids):
    plt.annotate(f"R{txt}", (operational_speeds[i], items_processed[i]), 
