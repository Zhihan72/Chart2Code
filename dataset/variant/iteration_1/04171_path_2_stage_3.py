...
# New shuffled colors
colors = ['#ff9896', '#c5b0d5', '#ffbb78', '#98df8a', '#aec7e8']

fig, ax = plt.subplots(figsize=(14, 8))
ax.stackplot(years, energy_data, labels=['Sol', 'Wnd', 'Hdr', 'Geo', 'Bio'], colors=colors, alpha=0.8)

ax.set_title("Energy Growth (2010-30)", fontsize=16, weight='bold')
ax.set_xlabel("Yr", fontsize=12)
ax.set_ylabel("Prod (TWh)", fontsize=12)

ax.legend(loc='upper left', fontsize=10, title='Sources')
ax.grid(True, which='both', linestyle='--', linewidth=0.5, alpha=0.7)

ax.annotate('2015: Solar rises', xy=(2015, solar_energy[5] / 2), xytext=(2013, 50),
            arrowprops=dict(facecolor='black', arrowstyle='->', lw=1.5),
            fontsize=10, fontweight='bold', color='darkslategray')
ax.annotate('2020: Wind > Hydro', xy=(2020, wind_energy[10] + hydro_energy[10] / 2), xytext=(2017, 250),
            arrowprops=dict(facecolor='black', arrowstyle='->', lw=1.5),
            fontsize=10, fontweight='bold', color='darkslategray')
ax.annotate('2025: Geo Boom', xy=(2025, geothermal_energy[15] + biomass_energy[15] / 2), xytext=(2022, 60),
            arrowprops=dict(facecolor='black', arrowstyle='->', lw=1.5),
            fontsize=10, fontweight='bold', color='darkslategray')
