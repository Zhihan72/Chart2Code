...
for i, (civ, advancements) in enumerate(tech_advancements.items()):
    axs[0].plot(years, advancements, marker=markers[i], linestyle=line_styles[i], color=colors[i], label=civ)
...
for i, (civ, population) in enumerate(population_growth.items()):
    axs[1].plot(years, population, marker=markers[i], linestyle=line_styles[i], color=colors[i], label=civ)
