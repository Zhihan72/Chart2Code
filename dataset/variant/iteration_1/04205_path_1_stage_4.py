...
geothermal = np.array([0, 0, 0, 0, 0, 50, 100, 150, 210, 280,
                       360, 450, 550, 680, 820, 950, 1080, 1240, 1400, 1550,
                       1680, 1820, 1950, 2050, 2180, 2300, 2400, 2500, 2550, 2650,
                       2700, 2750, 2800, 2850, 2900, 2950, 3000, 3050, 3100, 3150,
                       3200, 3250, 3300, 3350, 3400, 3450, 3500, 3550, 3600, 3650, 3700])

energy_data = np.row_stack([fossil_fuels, solar, wind, hydro, nuclear, geothermal])

fig, ax = plt.subplots(figsize=(14, 8))

colors = ['#87CEEB']
ax.stackplot(years, energy_data, colors=colors * 6, alpha=0.85)

total_energy = energy_data.sum(axis=0)
ax.plot(years, total_energy, color='black', linewidth=2.5, marker='o')

ax.set_title("Energy Production (1970-2020)\nFossil to Renewables", fontsize=16, fontweight='bold')
...
ax.annotate('Solar Start', xy=(1980, 100), xytext=(1985, 15000),
            arrowprops=dict(facecolor='black', arrowstyle='->'), fontsize=10)
ax.annotate('Wind Surge', xy=(2000, 700), xytext=(2005, 20000),
            arrowprops=dict(facecolor='black', arrowstyle='->'), fontsize=10)
ax.annotate('Hydro Stable', xy=(1995, 4500), xytext=(2000, 15000),
            arrowprops=dict(facecolor='black', arrowstyle='->'), fontsize=10)
