...
for i, color in enumerate(colors):
    if i == 0:
        bars = ax.bar(years, planting_data_t[:, i], color=color)
    else:
        bars = ax.bar(years, planting_data_t[:, i], bottom=np.sum(planting_data_t[:, :i], axis=1), color=color)

ax.yaxis.grid(False)  # Disable grid lines
