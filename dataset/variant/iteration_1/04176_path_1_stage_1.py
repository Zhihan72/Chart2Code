...
for i, data in enumerate([voice_assistants, smart_lighting, smart_thermostats]):
    city1 = axs[i].barh(position - bar_width, data[0], bar_width, label=cities[0], color=colors[0])
    city2 = axs[i].barh(position, data[1], bar_width, label=cities[1], color=colors[1])
    city3 = axs[i].barh(position + bar_width, data[2], bar_width, label=cities[2], color=colors[2])
