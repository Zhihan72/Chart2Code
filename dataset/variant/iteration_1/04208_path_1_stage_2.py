...
for i, ax in enumerate(axes.flat):
    wedges, _, autotexts = ax.pie(
        ...
        colors=colors, pctdistance=0.85, wedgeprops=dict(width=0.3), # Maintaining donut style with width
        explode=[0.05 if j == 0 else 0 for j in range(len(food_categories))]
    )
    
    # Add a white circle at the center to emphasize the donut effect
