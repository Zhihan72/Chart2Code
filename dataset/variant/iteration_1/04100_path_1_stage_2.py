...
for i, tree_type in enumerate(tree_types):
    if i == 0:
        bars = ax.bar(years, planting_data_t[:, i], color=single_color, edgecolor='black', label=tree_type)
    else:
        bars = ax.bar(years, planting_data_t[:, i], bottom=np.sum(planting_data_t[:, :i], axis=1), color=single_color, edgecolor='black', label=tree_type)

ax.set_xlabel('Year', fontsize=12)
ax.set_ylabel('Trees Planted', fontsize=12)
ax.set_title('Tree Planting: 2018-2022', fontsize=16, fontweight='bold')

ax.legend(loc='upper left', bbox_to_anchor=(1, 1), fontsize=12, title="Types", title_fontsize='13')
