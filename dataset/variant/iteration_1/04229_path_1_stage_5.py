...
festival_types = ['Music', 'Food', 'Art', 'Tech']
...
ax2 = plt.gca().twiny()
ax2.plot(avg_ratings, [index.mean() + 1.5 * bar_height] * len(avg_ratings), color='red', marker='s', linestyle='-', linewidth=2.5, label='Avg Rtg')

plt.title('2023 Metropolus Fest. Attend. & Rtg.', fontsize=18, fontweight='bold')
plt.ylabel('Months', fontsize=14)
plt.xlabel('Attendees (K)', fontsize=14)
ax2.set_xlabel('Avg Rtg (10)', fontsize=14)
...
plt.legend(title="Fest. Type", fontsize=12, loc='upper right', bbox_to_anchor=(1.1, 1))
