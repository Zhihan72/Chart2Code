...
# New data series for conservation efforts, represented as increments each year
conservation_efforts = np.array([0.5, 0.5, 0.7, 0.7, 0.8, 1.0, 1.2, 1.3, 1.5, 1.7, 1.9, 2.0, 2.2, 2.4, 2.5, 2.7, 3.0, 3.2, 3.4, 3.7, 4.0])

initial_forest_coverage = 100
net_deforestation = deforestation - reforestation - conservation_efforts
forest_coverage = initial_forest_coverage - np.cumsum(net_deforestation)

data_stack = np.vstack([deforestation, reforestation, conservation_efforts])

fig, ax = plt.subplots(figsize=(12, 8))

ax.stackplot(years, data_stack, colors=['#8B4513', '#2E8B57', '#4682B4'], alpha=0.6)
