...
for i, steps in enumerate(steps_data):
    ax.plot(days, steps, marker=markers[i], linestyle=linestyles[i], color=new_colors[i], label=f'Friend {i + 1}')
...
ax.legend(title="Friends", loc='lower right', fontsize=12, title_fontsize='13', shadow=True)
...
# Annotation with altered arrow properties
annotations = [
...
for text, x, y in annotations:
    ax.annotate(text, (x, y), xytext=(15, -25), textcoords='offset points',
                arrowprops=dict(arrowstyle="->, head_length=0.3", color='green'), fontsize=12, bbox=dict(boxstyle="round,pad=0.3", edgecolor="red", facecolor="lightyellow"))
