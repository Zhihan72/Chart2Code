...
for i, advancements in enumerate(tech_advancements.values()):
    axs[0].plot(years, advancements, marker=markers[i], linestyle=line_styles[i], color=colors[i])

axs[0].grid(False)

for i, population in enumerate(population_growth.values()):
    axs[1].plot(years, population, marker=markers[i], linestyle=line_styles[i], color=colors[i])
