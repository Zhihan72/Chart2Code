...
single_color = 'royalblue'  # Single color chosen to apply consistently
markers = ['o', 's', '^', 'd', 'v', '<', '>', '*']
line_styles = ['-', '--', '-.', ':']

for i, hero in enumerate(superheroes.keys()):
    ax1.plot(angles, superheroes[hero], color=single_color, linewidth=2, label=hero,
             linestyle=line_styles[i % len(line_styles)], marker=markers[i % len(markers)])
    ax1.fill(angles, superheroes[hero], color=single_color, alpha=0.2)
...
ax1.legend(loc='lower left', bbox_to_anchor=(1.05, 0.05), fontsize=10)
ax1.grid(False)
...
ax2.bar(mean_scores.keys(), mean_scores.values(), color=single_color, alpha=0.8, edgecolor='black', linewidth=1)
ax2.set_ylim(0, 10)
