...
# New set of colors for each planet's flight path
colors = ['blue', 'orange', 'green', 'purple', 'red']

# Plot each planet's flight path with new colors
for idx, (planet, distances) in enumerate(distance_to_planets.items()):
    ax.plot(time, distances, label=planet, marker='o', linestyle='-', linewidth=2, alpha=0.7, color=colors[idx])
