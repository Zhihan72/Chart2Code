...
activity_labels = ['Jogging', 'Biking', 'Leisure', 'Water Sports', 'Meditation']  # Changed labels
...
for i, (activity, color) in enumerate(zip(activities, colors)):
    bars = ax1.bar(years, activity, bottom=bottom, label=activity_labels[i], color=color, alpha=0.7, edgecolor='black', hatch='/')
    bottom += activity

ax2 = ax1.twinx()
ax2.plot(years, total_participation, color='purple', marker='s', linestyle='-.', linewidth=2.5, label='Aggregate Participation')  # Changed label

ax1.set_title("Outdoor Activity Trends\n(2017-2021)", fontsize=18, weight='bold', pad=20)  # Changed title
ax1.set_xlabel('Calendar Year', fontsize=14)  # Changed x-axis label
ax1.set_ylabel('Participant Count', fontsize=14)  # Changed y-axis label
ax2.set_ylabel('Total Sum', fontsize=14, color='black')  # Changed y2-axis label
