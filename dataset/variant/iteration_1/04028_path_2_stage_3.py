...
sns.kdeplot(y=months, x=temperatures_alpha_norm[:12], ax=ax[0], fill=True, color='blue', alpha=0.6, label='Alpha')
sns.kdeplot(y=months, x=temperatures_alpha_norm[12:], ax=ax[0], fill=True, color='red', alpha=0.6, label='Beta')
sns.kdeplot(y=months, x=temperatures_gamma_norm[:12], ax=ax[0], fill=True, color='purple', alpha=0.6, label='Gamma')
ax[0].set_title("Temp Density: Alpha vs Beta vs Gamma", fontsize=14, fontweight='bold')
ax[0].set_xlabel("Density", fontsize=12)
ax[0].set_ylabel("Month", fontsize=12)

# Plot horizontal rainfall density
sns.kdeplot(y=months, x=rainfall_alpha_norm[:12], ax=ax[1], fill=True, color='green', alpha=0.6, label='Alpha')
sns.kdeplot(y=months, x=rainfall_alpha_norm[12:], ax=ax[1], fill=True, color='orange', alpha=0.6, label='Beta')
sns.kdeplot(y=months, x=rainfall_gamma_norm[:12], ax=ax[1], fill=True, color='brown', alpha=0.6, label='Gamma')
ax[1].set_title("Rain Density: Alpha vs Beta vs Gamma", fontsize=14, fontweight='bold')
ax[1].set_xlabel("Density", fontsize=12)
