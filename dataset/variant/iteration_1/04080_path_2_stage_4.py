...
# Sort by the budget for 2021 in ascending order
...
for i, year in enumerate(years):
    ax.bar(x + (i - 1.5) * width, sorted_data[i], width, label=year, edgecolor='black', color=bar_colors[i])
...
ax.set_xticks(x)
ax.set_xticklabels(sorted_departments, rotation=30, ha='right')
...
rects_list = [ax.bar(x + (i - 1.5) * width, sorted_data[i], width, edgecolor='black') for i in range(len(years))]
annotate_bars(rects_list, sorted_data)
