...
# New diverging data setup
participants_pro = participants[:, [0, 1]].sum(axis=1)
participants_anti = -participants[:, [2, 3]].sum(axis=1)
...
# Plot positive half for Py and JS combined
ax.bar(x, participants_pro, width=half_bar_width, label='Py+JS', edgecolor='black', linestyle=':', hatch='/')

# Plot negative half for Ruby and C++ combined
ax.bar(x, participants_anti, width=half_bar_width, label='Ruby+C++', edgecolor='black', linestyle=':', hatch='\\')

# Add data labels for positive and negative bars
