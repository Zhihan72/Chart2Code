...
ax1.plot(years, action_sales, marker='s', linestyle='--', color='purple', label='Action', linewidth=2.5)
ax1.plot(years, comedy_sales, marker='^', linestyle='-.', color='darkgreen', label='Comedy', linewidth=2.5)
ax1.plot(years, drama_sales, marker='D', linestyle=':', color='navy', label='Drama', linewidth=2.5)
ax1.plot(years, scifi_sales, marker='*', linestyle='-', color='coral', label='Sci-Fi', linewidth=2.5)
...
for genre, (year, value) in annotations.items():
    ax1.annotate(f'{value}', xy=(year, value), xytext=(year + 0.5, value + 3),
                 arrowprops=dict(facecolor='red', linestyle='-', arrowstyle="->"))

total_sales = action_sales + comedy_sales + drama_sales + scifi_sales
ax2.plot(years, total_sales, marker='x', linestyle='-', color='black', linewidth=2.5)
