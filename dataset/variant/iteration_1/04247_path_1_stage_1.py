...
common_color = '#FF5733'  # Consistent color across all plots

# Plot data for each renewable energy source using a consistent color
ax.plot(years, solar, label='Solar Energy', marker='o', color=common_color, linewidth=2, linestyle='-')
ax.plot(years, wind, label='Wind Energy', marker='^', color=common_color, linewidth=2, linestyle='--')
ax.plot(years, hydro, label='Hydroelectric Energy', marker='s', color=common_color, linewidth=2, linestyle='-.')
...
for i, txt in enumerate(solar):
    ax.text(years[i], solar[i] + 2, str(txt), fontsize=9, ha='center', color=common_color)
for i, txt in enumerate(wind):
    ax.text(years[i], wind[i] + 2, str(txt), fontsize=9, ha='center', color=common_color)
for i, txt in enumerate(hydro):
    ax.text(years[i], hydro[i] - 3, str(txt), fontsize=9, ha='center', color=common_color)
