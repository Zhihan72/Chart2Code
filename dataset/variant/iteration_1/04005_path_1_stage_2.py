...
# Stacked histogram with altered stylistic elements
axs[0].hist([np.arange(len(seasons))] * len(data), bins=np.arange(len(seasons) + 1), weights=data, 
            label=appliance_labels, histtype='barstacked', alpha=0.6, edgecolor='navy', linestyle='dashed')
...
# Percentage share line plot with altered stylistic elements
marker_styles = ['s', '^', 'd', '*']
line_styles = [':', '-', '-.', '--']

for idx, percentage in enumerate(percentage_data):
    axs[1].plot(seasons, percentage, marker=marker_styles[idx], label=appliance_labels[idx], linewidth=2, linestyle=line_styles[idx])
