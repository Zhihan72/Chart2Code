...
discoveries_per_year = np.array(discoveries) / np.array([40, 10, 20, 15, 12, 12, 10, 60])

colors = ['lightgreen', 'lightcoral', 'lightsalmon', 'skyblue', 'orchid', 'orange', 'lightsteelblue', 'peachpuff']

fig, ax1 = plt.subplots(figsize=(14, 8))
bars = ax1.bar(missions, discoveries, color=colors)
...
ax2 = ax1.twinx()
ax2.plot(missions, discoveries_per_year, color='darkorange', marker='o', linestyle='-', linewidth=2, markersize=8)
...
for mission, label in annotations.items():
    ...
    ax1.annotate(label, (missions[idx], discoveries[idx]), xytext=(0, 10), textcoords='offset points', arrowprops=dict(arrowstyle='->', color='gray'), ha='center', fontsize=10, color='maroon')
