...
for i, event in enumerate(celestial_events):
    ...
    bars = ax1.bar(index + i * bar_width, counts, bar_width, label=event, hatch=hatches[i])
...
ax1.yaxis.grid(True, linestyle='-', which='major', color='blue', alpha=0.3)  # Change grid style and color
ax1.legend(title='Celestial Events', fontsize=10, title_fontsize='12', loc='lower left')  # Change legend location

# Changed stacked bar chart styling
...
for i, (city, color) in enumerate(zip(cities, colors)):
    ax2.bar(months, monthly_sightings[city], bottom=bottom_values, label=city, color=color, linestyle='--')  # Change line style
...
ax2.set_ylim(0, 10)
ax2.yaxis.grid(True, linestyle=':', which='major', color='green', alpha=0.5)  # Change grid style and color
ax2.legend(title='Cities', fontsize=10, title_fontsize='12', loc='lower right')  # Change legend location
