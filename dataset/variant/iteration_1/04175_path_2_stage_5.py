...
# Sort the data by study_hours in descending order
sorted_indices = np.argsort(study_hours)[::-1]
depts = [depts[i] for i in sorted_indices]
study_hours = [study_hours[i] for i in sorted_indices]
gpas = [gpas[i] for i in sorted_indices]

bar_colors = ['#FF7F50', '#6A5ACD', '#FFD700', '#DC143C', '#20B2AA']  # Colors reshuffle according to sorted indices
line_color = '#FF7F50'

fig, ax1 = plt.subplots(figsize=(15, 8))

# Bar chart with sorted data
