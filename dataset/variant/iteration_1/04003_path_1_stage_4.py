...
fruits = ["Apples", "Bananas", "Oranges", "Grapes", "Strawberries", "Peaches", "Cherries"]
...
peach_sales = np.array([100, 120, 130, 140, 150])  # New data for Peaches
cherry_sales = np.array([95, 105, 98, 102, 110])   # New data for Cherries 

# Stack the sales data for easy bar plotting
sales_data = np.vstack([apple_sales, banana_sales, orange_sales, grape_sales, strawberry_sales, peach_sales, cherry_sales])

# Create a figure and axis
fig, ax = plt.subplots(figsize=(16, 9))

# Define bar positions and width
bar_width = 0.1  # Adjusted for new data
x = np.arange(len(years))

# Plot each fruit's sales data as grouped bars with unique colors for each
colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2']  # Added new colors
