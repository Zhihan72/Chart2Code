...
edge_weights = {tuple(sorted(edge)): np.random.randint(1, 10) for edge in G.edges()}
...
# Add edge labels
edge_labels = {tuple(sorted(edge)): str(weight) for edge, weight in edge_weights.items()}
nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, ax=axes[0], font_color='red', font_size=8)

axes[0].set_title("AI Research Network\nCollab Strength", fontsize=16, fontweight='bold', pad=20)
axes[0].axis('off')

# Calculate collaboration strength for each node
...
# Plot collaboration strength as a bar chart
