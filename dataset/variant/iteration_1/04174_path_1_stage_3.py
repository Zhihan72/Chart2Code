...
colors = ['c', 'm', 'y', 'k']
bottom_cumulative = np.zeros(len(months))

for idx, age_idx in enumerate(age_group_indices):
    ax[1].bar(months, steps_per_month[age_idx], bottom=bottom_cumulative, color=colors[idx], label=f'Age {age_groups[age_idx]}')
    bottom_cumulative += steps_per_month[age_idx]
