...
for i in range(len(categories)):
    bars = ax1.bar(positions + i * bar_width, usage_data[:, i], bar_width, color=consistent_color, edgecolor='gray', alpha=0.85)
    for bar in bars:
...
for i in range(len(categories)):
    ax2.barh(positions, cumulative_data[:, i], bar_width, color=consistent_color, edgecolor='gray', alpha=0.85)
