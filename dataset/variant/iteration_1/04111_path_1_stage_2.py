...
# Data sorted in ascending order of total number of restaurants
data = np.array([
    [80, 75, 60, 95],   # Sydney
    [90, 85, 70, 100],  # Paris
...
# Sum restaurants by city for sorting
...
for i, cuisine in enumerate(cuisines):
    ax.bar(r + i * bar_width, data[:, i], width=bar_width, edgecolor='grey', label=cuisine)
...
bars = [ax.bar(r + i * bar_width, data[:, i], width=bar_width, edgecolor='grey') for i in range(len(cuisines))]
for bar in bars:
    add_labels(bar)
