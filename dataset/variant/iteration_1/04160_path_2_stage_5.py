...
ax1.plot(months, avg_temps, label="Avg Temp", linestyle='-', color='purple', marker='o')
ax1.axhline(y=avg_temps.mean(), color='grey', linestyle='--', linewidth=2, label="Yr Avg")
ax1.set_title("Avg Mo Temps - 2023", fontsize=14, fontweight='bold')
ax1.set_xlabel("Mo", fontsize=12)
ax1.set_ylabel("Avg Temp (°C)", fontsize=12)
ax1.legend(loc="lower left", fontsize=10, frameon=False)
ax1.grid(True, linestyle=':', alpha=0.9)

ax2.plot(months, new_york_temps, label="NY", linestyle='--', color='orange', marker='^')
ax2.plot(months, tokyo_temps, label="Tokyo", linestyle='-.', color='blue', marker='*')
ax2.plot(months, paris_temps, label="Paris", linestyle='-', color='green', marker='p')
ax2.plot(months, sydney_temps, label="Sydney", linestyle=':', color='red', marker='h')
...
for i, (city, temps) in enumerate(cities_data.items()):
    max_temp = max(temps)
    min_temp = min(temps)
    ax2.annotate(f'{max_temp}°C', xy=(months[temps.argmax()], max_temp), 
                 xytext=(months[temps.argmax()], max_temp + 3), ha='center',
                 color=colors[i], fontsize=10, arrowprops=dict(facecolor=colors[i], arrowstyle='->'))
    ax2.annotate(f'{min_temp}°C', xy=(months[temps.argmin()], min_temp), 
                 xytext=(months[temps.argmin()], min_temp - 5), ha='center',
                 color=colors[i], fontsize=10, arrowprops=dict(facecolor=colors[i], arrowstyle='->'))

ax2.set_title("2023 Temps - NY, Tokyo, Paris, Sydney", fontsize=16, fontweight='bold')
