...
# Use a single color 'teal' consistently across the plots
common_color = 'teal'

ax1.hist(all_months, bins=12, color=common_color, edgecolor='brown', alpha=0.6, linestyle='-.', linewidth=1.5)
...
for i, (year, observations) in enumerate(zip(years, monthly_observations)):
    ax2.scatter(months + i * 0.1, observations, s=60, alpha=0.6, marker=marker_styles[i % len(marker_styles)], color=common_color)
