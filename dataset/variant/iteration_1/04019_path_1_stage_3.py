...
colors = ['#8b4513', '#f3e5ab', '#fc5a8d', '#c0c0c0', '#98ff98']
markers = ['s', '^']
linestyles = ['-.', (0, (3, 1, 1, 1))]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))

# Swap subplot contents
years = np.arange(2013, 2024)
vanilla_growth = [20, 22, 23, 25, 26, 27, 29, 30, 32, 33, 34]

ax1.plot(years, vanilla_growth, marker=markers[0], linestyle=linestyles[0], color=colors[1], linewidth=2, markersize=8, label='Vanilla')

for i, growth in enumerate(vanilla_growth):
    ax1.text(years[i], growth + 0.5, f'{growth}%', ha='center', fontsize=10)

ax1.set_title("Vanilla Flavor Growth (2013-2023)", fontsize=14, fontweight='bold')
...
for idx, flavor in enumerate(flavors):
    ax2.bar(continents, flavor_data_pos[:, idx], label=flavor, color=colors[idx], edgecolor='gray', alpha=0.7)
...
ax2.set_title("Global Ice Cream Flavor Popularity by Continent (2023)", fontsize=16, fontweight='bold')
ax2.set_xlabel("Continent", fontsize=12)
ax2.set_ylabel("Difference from 25% Popularity", fontsize=12)
ax2.axhline(0, color='black', linewidth=0.9)
ax2.legend(title='Ice Cream Flavors', loc='lower left')
