...
# Assign new colors for zones
zone_colors = ['seagreen', 'darkorange', 'slateblue', 'purple', 'teal', 'coral', 'sienna', 'gold']  # Shuffled colors

# Plot sorted bar chart for zones
ax2.barh(np.arange(len(sorted_zones)), sorted_activity_by_zone, color=zone_colors[:len(sorted_zones)])
...
# Assign new colors for days
day_colors = ['navy', 'green', 'crimson', 'orchid', 'peru', 'skyblue', 'olive']  # Shuffled colors

# Plot sorted bar chart for days
ax2_2 = ax2.twinx()
ax2_2.bar(np.arange(len(sorted_days)), sorted_activity_by_day, alpha=0.7, color=day_colors[:len(sorted_days)], width=0.3, align='center')
