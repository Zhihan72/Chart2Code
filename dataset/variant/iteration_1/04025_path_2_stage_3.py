...
squarify.plot(sizes=sizes, label=labels, color=colors * len(data), alpha=0.8, edgecolor=None, text_kwargs={'fontsize': 8, 'wrap': True})
...
for i, amenity in enumerate(amenities):
    ax.bar(planets, totals[i], bottom=bottom, color=colors[i % len(colors)])
    bottom += totals[i]
