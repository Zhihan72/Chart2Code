...
# Backstory:
# This chart represents the monthly gaming hours of four friends who are dedicated gamers. 
# They have kept a record of their gaming habits over a year, and the box plot will show 
# the distribution of their monthly gaming hours. Additionally, another subplot will display
# their favorite gaming genres through a bar chart.

# Data collection for monthly gaming hours
friends = ['Alice', 'Bob', 'Charlie', 'Diana']

# Monthly gaming hours recorded over the past year
...
# Favorite gaming genres data (in terms of percentage of total gaming time)
...
ax1.legend(handles=legend_elements, title='Friends', loc='upper left', fontsize=10)
...
ax2.bar(X + offset[0], alice_genres, width=bar_width, label='Alice', color=colors[0])
ax2.bar(X + offset[1], bob_genres, width=bar_width, label='Bob', color=colors[1])
ax2.bar(X + offset[2], charlie_genres, width=bar_width, label='Charlie', color=colors[2])
ax2.bar(X + offset[3], diana_genres, width=bar_width, label='Diana', color=colors[3])
