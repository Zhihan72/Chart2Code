...
# Average data across all adventurers
average_abilities = [
    np.mean([arthur[i], elena[i], morgana[i]]) 
    for i in range(num_vars)
]

# Function to create the radar chart for a given adventurer
...
create_radar_chart(polar_ax, arthur, 'b', 'Arthur the Brave')
create_radar_chart(polar_ax, elena, 'r', 'Elena the Swift')
create_radar_chart(polar_ax, morgana, 'g', 'Morgana the Wise')
...
ax2.bar(categories, average_abilities, color=['b', 'r', 'g', 'purple', 'orange'])
ax2.set_ylim(0, 10)
