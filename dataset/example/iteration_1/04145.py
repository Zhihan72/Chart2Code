...
# Backstory:
# The local community garden is conducting a seasonal study to understand the productivity of different crops. They've taken measurements of crop yield (in kilograms) for tomatoes, carrots, and lettuce over three seasons: spring, summer, and autumn. They want to visualize this data to determine which crop thrives best in each season and also to compare the seasonal averages.
...
axes[0].legend(handles, crops, title='Crops', loc='upper left', fontsize=12)
...
for i, crop in enumerate(crops):
    axes[1].plot(x_pos, average_values[i], marker='o', linestyle='-', linewidth=2, markersize=8, label=crop, color=colors[i])
...
axes[1].legend(title='Crops', fontsize=12)
