...
# Create artificial data for average temperatures (in °C) for fictional regions
...
ax.plot(years, north_region_temp, 'o-', label='North Region', color='blue', linewidth=2.5)
ax.plot(years, south_region_temp, 's-', label='South Region', color='red', linewidth=2.5)
ax.plot(years, central_region_temp, '^-', label='Central Region', color='green', linewidth=2.5)
...
# Annotate important points (e.g., peaks and significant years)
...
for year, n_temp, s_temp, c_temp in zip(years, north_region_temp, south_region_temp, central_region_temp):
    ...
    ax.annotate(f'{c_temp:.1f}', xy=(year, c_temp), textcoords="offset points", xytext=(0,5), ha='center', fontsize=8, color='green')

# Customize grid and axis formatting
