...
# Defining performance factors for Superhero Abilities
abilities = ['Strength', 'Speed', 'Intelligence', 'Agility', 'Stamina', 'Stealth']
num_abilities = len(abilities)

# Creating performance data for superheroes
superheroes = {
...
# Calculate mean scores for bar chart
...
colors = plt.cm.viridis(np.linspace(0, 1, len(superheroes)))  # Can be adjusted for variety
for (hero, color) in zip(superheroes.keys(), colors):
    ax1.plot(angles, superheroes[hero], color=color, linewidth=2, linestyle='solid', label=hero)
