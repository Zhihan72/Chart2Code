...
# Backstory:
# In a fictional underwater city, various forms of energy are being utilized to sustain the city’s needs. This script visualizes the power sources for the city and compares the efficiency of these power sources in a secondary plot.

# Define the sectors and their respective power sources
...
# Energy distribution by type in each sector (percentages)
energy_data = [
    [50, 10, 20, 20],  # Tidal Power
    [10, 60, 20, 10],  # Geothermal Energy
    [20, 20, 40, 20],  # Hydrogen Fuel Cells
    [20, 10, 20, 50]   # Bioenergy
]

# Efficiency ratios (arbitrary units)
...
# Explode parameter to highlight key sections
explode = (0.1, 0, 0, 0)
...
# Add a legend for the power sources
fig.legend(wedges, power_sources, loc='center right', fontsize=12, title='Power Sources')

# Create an additional subplot for efficiency comparison
ax_eff = fig.add_subplot(224)
bars = ax_eff.bar(sectors, efficiency_ratios, color=colors, edgecolor='black', linewidth=1.5)
