...
# Backstory: The galactic study of alien plant growth on different planets.
# Scientists recorded growth heights and diameters of a specific alien plant species over several years on three different planets: Xenia, Zog, and Trogar.
...
# Plotting the scatter charts
ax1.scatter(xenia_height, xenia_diameter, color='blue', edgecolor='black', alpha=0.7, s=100, label='Xenia')
ax1.scatter(zog_height, zog_diameter, color='green', edgecolor='black', alpha=0.7, s=100, label='Zog')
ax1.scatter(trogar_height, trogar_diameter, color='red', edgecolor='black', alpha=0.7, s=100, label='Trogar')
...
for i in range(0, len(years), 2):
    ...
    ax1.annotate(f'Year {years[i]}', (zog_height[i], zog_diameter[i]), textcoords="offset points", xytext=(0,10), ha='center', fontsize=9)
    ax1.annotate(f'Year {years[i]}', (trogar_height[i], trogar_diameter[i]), textcoords="offset points", xytext=(0,10), ha='center', fontsize=9)

# Auxiliary plot: Comparison of average height and diameter
...
bars1 = ax2.bar(index, average_heights, bar_width, label='Avg Height (cm)', color='b', alpha=0.7)
bars2 = ax2.bar(index + bar_width, average_diameters, bar_width, label='Avg Diameter (cm)', color='r', alpha=0.7)
...
ax2.set_xticklabels(['Xenia', 'Zog', 'Trogar'])
ax2.legend()
