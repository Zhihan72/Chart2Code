...
# Backstory
# We are conducting a study about monthly average sales performance in different product categories across various regions. 
# This study will help the company understand regional preferences and seasonal influences on product sales.

# Define regions and product categories
regions = ["North", "South", "East", "West"]
categories = ["Electronics", "Apparel", "Home Goods", "Books", "Toys", "Groceries"]

# Constructing sales data (monthly average sales for each region-category combination)
# Data is fictional and designed to highlight regional sales trends
sales_data = np.array([
...
# Creating some additional data for subplot to show product trends over the months
...
# Convert product trends to matrix for heatmap
trend_data = np.array([product_trends[category] for category in categories])

# Create a figure and subplots
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))

# Plot the main heatmap for regional sales data
cax = ax1.matshow(sales_data, cmap='viridis', aspect='auto')
...
ax1.set_title('Average Monthly Sales by Region and Category', pad=15)

# Annotate heatmap cells with sales values
for i in range(sales_data.shape[0]):
    for j in range(sales_data.shape[1]):
        ax1.text(j, i, f"{sales_data[i, j]:.0f}", ha='center', va='center', color='white' if sales_data[i, j] > 100 else 'black')
