...
# Backstory:
# WindCo is a fictional company specializing in wind energy. Over a decade, it has increased its wind farm installations in three different regions.
# The goal is to visualize the annual installation capacity (in Megawatts) of wind farms in each region and the overall increase in capacity over the years.
...
# Plot line graphs for each region
ax1.plot(years, north_region, marker='o', linestyle='-', color='blue', linewidth=2, label='North Region')
ax1.plot(years, central_region, marker='s', linestyle='--', color='green', linewidth=2, label='Central Region')
ax1.plot(years, south_region, marker='^', linestyle='-.', color='red', linewidth=2, label='South Region')

# Plot the total capacity line graph on a secondary y-axis
ax2 = ax1.twinx()
