...
# Backstory: Analyzing the sales performance of our top 5 products over the past 12 months at a fictional company.
...
# Plot each product's sales in a stacked bar format
colors = ['#FF6347', '#FFD700', '#4B0082', '#98FB98', '#9370DB']

bars = []
for i, product in enumerate(products):
    if i == 0:
        bars.append(ax.bar(index, product_sales[i], bar_width, color=colors[i], label=product))
    else:
        bars.append(ax.bar(index, product_sales[i], bar_width, bottom=cumulative_sales[i-1], color=colors[i], label=product))
