...
# Backstory: Solar System Mission Project
# The chart represents the progress in distance (in million kilometers) traveled by five different spacecraft launched from Earth over 10 years.
...
# Explicitly created data for storytelling
...
ax.plot(years, voyager_1, marker='o', linestyle='-', linewidth=2, label='Voyager 1')
ax.plot(years, voyager_2, marker='^', linestyle='--', linewidth=2, label='Voyager 2')
ax.plot(years, new_horizons, marker='s', linestyle='-.', linewidth=2, label='New Horizons')
ax.plot(years, juno, marker='D', linestyle=':', linewidth=2, label='Juno')
ax.plot(years, cassini, marker='x', linestyle='-', linewidth=2, label='Cassini')
...
for i, d in enumerate([voyager_1, voyager_2, new_horizons, juno, cassini]):
    ax.text(years[-1] + 0.2, d[-1], f'{d[-1]:.1f}M km', horizontalalignment='left', size=10, color=ax.lines[i].get_color())
