...
# The story: In an alternate universe, different factions of wizards are competing in the Wizard's Cup, demonstrating their magical prowess across five categories. The factions are evaluated across these categories to determine the ultimate wizarding power balance.
...
for (name, values), color in zip(data.items(), colors):
    ax.fill(angles, values, color=color, alpha=0.25, label=name)
...
ax.set_title('Wizard\'s Cup Battle of the Factions\nMagical Prowess Across Various Categories', size=14, weight='bold', pad=20)
ax.legend(loc='upper right', bbox_to_anchor=(1.2, 1.1), title="Factions", frameon=False)
