...
# Backstory: We are tracking the rise of smartphone screen sizes over the years. 
# The chart will plot the average screen sizes from 2010 to 2020 with specific models highlighted.
...
ax.plot(years, avg_screen_sizes, color='blue', marker='o', linestyle='-', linewidth=2, label='Avg Screen Size')

# Highlight notable smartphone models
for model, (year, size) in notable_models.items():
    ax.plot(year, size, 'ro')  # Red dot for notable models
    ax.text(year, size + 0.1, model, color='red', ha='center', fontsize=10)
