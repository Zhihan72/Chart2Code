import matplotlib.pyplot as plt

# Hypothetical data on various fruit and vegetable consumption (servings per week) among individuals of different age groups
kids_data = [5, 7, 6, 4, 5, 6, 8, 9, 10, 7, 5, 6, 4, 6, 5, 7, 8, 11, 12]
teens_data = [6, 8, 9, 7, 7, 6, 8, 10, 12, 11, 10, 9, 7, 9, 8, 10, 8, 13, 14]
adults_data = [9, 10, 12, 11, 12, 14, 13, 12, 14, 15, 14, 13, 12, 14, 15, 17, 18, 19, 20]
seniors_data = [8, 10, 9, 7, 8, 9, 11, 12, 10, 11, 9, 8, 7, 10, 9, 8, 10, 11, 12]

# Group data together into a list for plotting
data = [kids_data, teens_data, adults_data, seniors_data]

# Corresponding age group labels
age_labels = ['Kids (5-12)', 'Teens (13-19)', 'Adults (20-64)', 'Seniors (65+)']

# Initialize the plot
plt.figure(figsize=(12, 7))

# Create vertical box plots
box = plt.boxplot(data, patch_artist=True, notch=True, vert=True, labels=age_labels)

# Define a color palette for each group
colors = ['#FFC300', '#FF5733', '#C70039', '#900C3F']  # Yellow, Orange, Red, Dark Red
for patch, color in zip(box['boxes'], colors):
    patch.set_facecolor(color)

# Customize whiskers, caps, medians, and outliers
plt.setp(box['whiskers'], color='black', linewidth=1.5)
plt.setp(box['caps'], color='black', linewidth=1.5)
plt.setp(box['medians'], color='blue', linewidth=2)
plt.setp(box['fliers'], marker='o', color='green', alpha=0.5)

# Add title and labels
plt.title("Weekly Fruit and Vegetable Consumption\nDifferent Age Groups", fontsize=16, fontweight='bold')
plt.xlabel("Age Groups", fontsize=12)
plt.ylabel("Servings per Week", fontsize=12)

# Add grid lines for easier reading
plt.grid(axis='y', linestyle='--', alpha=0.7)

# Adjust layout to prevent text overlap
plt.tight_layout()

# Display the plot
plt.show()