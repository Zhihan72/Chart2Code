...
# Backstory: The chart illustrates a fantasy land known as "Solaris Kingdom." 
# Each node represents an ancient city, and edges depict trade routes between them.
# The weights of the edges represent the trade volume between these cities.
...
nx.draw_networkx_nodes(G, pos, node_size=1200, node_color='lightcoral', alpha=0.9, edgecolors='k', ax=ax)
...
nx.draw_networkx_edges(G, pos, edgelist=trade_routes, arrowstyle='-|>', arrowsize=15, edge_color=edges_weights, 
                       edge_cmap=plt.cm.autumn, width=2, ax=ax)
...
sm = mpl.cm.ScalarMappable(cmap=plt.cm.autumn, norm=plt.Normalize(vmin=min(edges_weights), vmax=max(edges_weights)))
sm.set_array([])
