...
# Backstory: Animal Husbandry in Rural Regions
# In rural areas, tracking the birthing rates of livestock is important for agricultural planning and resource allocation.
# The chart will display histogram data about birth rates of different animal species over the past year,
# with additional plots highlighting weekly variations and comparisons between regions.
...
for i in range(len(species)):
    axs[0, 1].plot(weeks, weekly_birth_rates_region_A[:, i], marker='o', label=species[i])
...
for i in range(len(species)):
    axs[1, 1].plot(weeks, weekly_birth_rate_ratio[:, i], marker='x', label=species[i])
